...
8. `admin` **학원 일지**(**관리자 전용, 전체 모니터링 허브**) — 세그먼트 **5개** `S.adminSeg`: **한눈에**(overview 대시보드: KPI 4개+선생님 현황+수강료 요약+데이터백업)·**학생관리**(student=반별 보드, isSchedStu 마크업 복제→`isAdminStudent`, board/boardGroups 가드에 admin+student 추가)·**주간/월간**(schedule=주간+월간+반별일정 캘린더, isSchedJournal 복제→`isAdminSchedule`, schedWeek/schedSummary/schedClasses 가드에 admin+schedule 가산)·수강료 장부(`isFee`)·데이터(`isData`). (v32.439/440) 학원 일지는 필터 제외(항상 전체). ⚠️ 보드·캘린더 마크업이 선생님 일지와 **복제(중복)** 상태 — 한쪽 수정 시 양쪽 반영 필요.
...
- ✅ **학생 삭제 = 표식(tombstone)으로 유지**(v32.462, 위 트레이드오프의 예외): union 병합이 삭제를 되살리던 문제 해결. `removeStudent(cid,sid)`가 삭제 id를 `data.deletedStudents[]`에 기록 → `mergeAppData`가 이 배열을 `_hisUnionStr`로 합집합(`out.deletedStudents`)한 뒤 **병합된 out.classes에서 해당 id 학생을 filter로 제거**(함수 끝 catch-all 직전, try/catch 폴백). 양방향·순서무관(헤드리스 실행 검증). 표식 없는 학생은 기존대로 union 보존. ⚠️ **다른 컬렉션(상담·성적·출결 등)의 삭제는 여전히 tombstone 없음** → 되살아날 수 있으니 필요 시 같은 패턴(전용 deletedX 배열+merge strip) 확장. **퇴원(withdrawn)은 삭제 아님**(데이터 유지, 표식 안 함).
