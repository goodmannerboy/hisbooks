...
**v33.208 응시생 수 삭제(원장 «응시생 몇 명인지는 적지 말고 평균만»)**: 학원 시험 통지서(#schoolcap) `acadRep.c.nLabel` = «평균»(예전 «응시생 N명 평균»). 선생님용 반 결과판 «응시» 칸은 유지(학부모 전송 없음). 성적통지서(#examcap)는 원래 인원 표기 없음.
...
통지서 맞춤 = `[data-exfit]` 상자 안 `[data-exfit-in]` 을 680px 고정 후 `_exFit()`(componentDidUpdate + 창 크기 변경)가 `transform:scale` — 기본 «전체 한눈에»(칸 높이·폭에 맞춤), «크게 보기»(`examFitMode:'w'`, 폭만). ⚠️ 축소는 **바깥 상자에만** — 캡처 대상 #schoolcap/#examcap 자체에는 transform 을 걸지 않아 카톡 이미지 680px 그대로(복제본 캡처). 검증 `wide209_e2e.py` 12항목.
