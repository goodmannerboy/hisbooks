...
- **서술형·복수정답은 자동 채점에서 제외**: 문항에 `sub:1`(+`multi:1`, 모범답 `ans`) → `_esQs` 가 학원 시험에서 `sub` 문항을 뺌 → 점수·통계·통지서·답안 지도가 전부 객관식만(배점 없으면 균등 100점). OMR 카드는 한 문항에 한 칸만 읽고 두 칸은 «확인 필요»로 빠지므로 복수정답은 구조상 채점 불가. 정답키 줄에 «서술형/복수정답» 표식(`data-q-sub`, 흐리게, title=모범답). **그 줄에서 정답 칸을 직접 누르면** `setExamQ('key')` 가 `sub` 를 풀어 객관식으로 편입·재채점.
- 검증 `list220_e2e.py` 12항목(실제 파일 올리기: 30문항·정답 20·서술형 8·복수 2 / 20개 다 맞으면 100 / 2개 틀리면 90 / 통지서 18/20 / 13번 직접 지정 → 21문항 85.7).
⚠️ 남은 것(원장 판단 대기): 서술형을 선생님이 O/X·부분점수로 넣어 총점에 합치는 기능은 없다. 복수정답을 OMR 두 칸 마킹으로 채점하는 기능도 없다.
