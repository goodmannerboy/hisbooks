...
- ⚠ D-day 배지는 시험이 **약 30일 안**일 때만 뜬다(`_examCalc`). 하니스에서 D-31로 시드하면 배지가 없어 «안 보인다»로 오판하기 쉽다 — D-10 등 가까운 날짜로 시드할 것. 학교 키는 `_schoolKey(_stuSchool(stu))` 로 만들어야 `examSchool` 과 맞는다(«포항동지고 1학년» → «포항동지고»).
