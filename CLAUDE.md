...
- 구현: ①레이더→«영역별 성취도» 섹션 우측 컴팩트 통합(2열 그리드, rv `ltRadarLabelsC` — 같은 svg viewBox 축소
  ...
  기준(`ltAvgGradeNum` 재정의; 성취도 헤더 «평균 N%»만 평균 유지 — ⚠️ 등급 기준은 v33.263 부터 OMR 채점 학생 = 판정 반, §8-61) ⑥수기(제출 없음/10문항 미만)=기존 레이아웃.
...
- ⚠️ **템플릿 `ondragover` 는 React 에 `onDragover`(소문자 o)로 붙어 무시된다(v33.207 교훈)** — 원장 신고 «파일 끌어넣기 작동 안 함». dragover 기본 동작이 안 막히면 실제 브라우저는 drop 을 상자에 아예 넘기지 않는다(`ondrop` 은 `onDrop` 으로 정상 연결돼 합성 이벤트 테스트는 통과해 버림). 수리 = 상자에 `data-dropzone="1"` 을 달면 componentDidMount 의 문서 dragover 리스너(`[data-feedrop],[data-dropzone]`)가 preventDefault. **새 끌어놓기 상자는 반드시 `data-dropzone="1"`**, 테스트는 dragover 의 `defaultPrevented` 까지 볼 것(`drop_probe.py`). 적용 상자: 정답·문제 PDF · OMR 스캔 · 레벨테스트 OMR.
...
### 8-9d. 일간일지 숨은 캡처카드 게이팅 + 남은 병목 판정 (2026-09-05, v33.154)
- **일간 `#cap-{sid}`** = `classReportCap`(미리보기 중인 학생 + 전송 큐의 **다음** 학생(미리 렌더해 전진 시 깜빡임 0) +
  행 복사 대기 학생 `capOneSid`, «전체 PNG 일괄 저장» 중엔 `capAllOn` 으로 전원). `classReport` 자체는 그대로(카운트·downloadAll 용).
  `copyRow` 는 카드가 없으면 `_capRetry` 저장 후 대기, `componentDidUpdate` 가 카드 생기면 재호출 + 미리보기 재읽기 1회(`_capSeenP`).
  A/B(오늘 기록 8명 저장 후 코멘트 타이핑, 6배 스로틀): 중앙 549/633→360/462ms, 최대 2,029→940ms, DOM 2,093→1,111노드.
- ⚠ **게이트 탐침도 같이 바뀜**: `his-homework` «학부모 카드에 과제 항목이 나옴»은 이제 `previewStudentId` 를 세팅해 카드를
  띄운 뒤 읽는다. 숨은 캡처카드를 직접 `getElementById` 로 읽는 새 검사를 쓰면 안 된다 — 실사용 경로(미리보기·복사·전송)로 열 것.
- **선생님일지·학생관리(1,245노드)는 숨은 영역이 0** — 보이는 타일 64개+달력이 전부라 게이팅 대상이 없다(실측: offzone 0).
  남은 비용은 템플릿 엔진의 요소당 속성 해석 비용이며, 줄이려면 가상화(보이는 반만 렌더)나 렌더 경계 도입이 필요 — 별도 설계 필요.
- ⚠ 게이트를 A/B 측정과 **동시에 돌리지 말 것** — CPU 경합으로 타이밍 기반 항목이 무더기 실패(8건)로 보였다(단독 재실행 시 전부 통과).

### 8-9e. 선생님일지·학생관리 계산 결과 캐시 (B안) + 성적 추이 단일 차트 (2026-09-05, v33.155)
- **메모 도우미** `_mz(obj, key, fn)`(WeakMap: 객체 신원 + 문자열 키) / `_mzs(name, key, fn)`(문자열 키 Map, 4,000건 넘으면 비움).
  데이터가 바뀌면 `data`·학생 객체가 통째로 새로 복제되므로(JSON clone) 캐시는 자동 무효화된다. 오늘 날짜에 기대는 것은 키에 `today()` 포함.
- 캐시된 것: `_examDdayStu/_examDdayClass/_examChip/examAgg/_csWhoLine`(data 신원), `_notStarted/_isHigh3/_stuGrade`(학생 객체 신원),
  `_schoolKey/_schoolDisp/_dnum`(문자열), `today()` 1초. 원본은 `…Raw`로 이름만 바뀜 — **새 계산 함수를 만들 때 순수(입력→출력)면 같은 패턴으로 감쌀 것**.
  보드 타일 `_sexs`는 전체 exams 스캔 대신 `_exsS(data, sid)` 인덱스.
- ⚠ 캐시 함수 안에서 `this.state`를 읽으면 키에 그 값을 넣어야 한다(안 넣으면 화면 전환 후 옛 값). 반환 객체는 공유되므로 **호출한 쪽에서 mutate 금지**.
- **성적 추이 차트는 1개**: x축 = 세 종류를 합쳐 날짜순 등간격(`trendPts`), 종류별 선(`trendCats[].trendPath`) 색 = 모의고사 #0C4631 · 학교시험 #8A6520 · 학원시험 #2B6CB0,
  범례에 종류별 횟수, 말풍선은 10회 이하 전부 / 넘으면 종류별 마지막 점만, 날짜 라벨은 끝점부터 `ceil(n/10)` 간격. 종류별 분할 카드는 폐기.
- ⚠ 템플릿 블록을 파이썬으로 넣을 때 `</` → `<\/` 변환에서 `<`를 빠뜨리면 마크업이 조용히 깨진다(닫는 태그가 텍스트가 되어 svg가 3중 중첩). 넣은 뒤 반드시 DOM 개수 탐침(E2E «차트 1개·선 3개·점 7개»).

### 8-9f. 차트 파스텔 1안 + 월간 우측 패널 «영역별 성장 비교» 미표시 수리 (2026-09-06, v33.156)
- **팔레트 1안(세이지·허니·스카이)**: 선 모의고사 #8CC4A6 · 학교시험 #E9C46A · 학원시험 #8FB8E3, 말풍선 = 연한 바탕(#DDF0E5/#FBF0C9/#DFEAF8)+진한 글자(#1E5A3E/#7A5410/#1F4C7E)+1px 선색 테두리,
  점 = 흰 속+색 테두리 2.5, 격자 #EDE7D6, 양 끝 말풍선은 padL+18~W-padR-18 안으로. 취약 유형 막대 = 코랄 #E9A192 / 허니 #E9C46A / 샌드 #EBDCA6, «최근 시험» 배지 #9A3F33/#FBE9E3.
  월간: 막대 #8CC4A6(트랙 #EFECE0), 추이 선 #8CC4A6·면 rgba(140,196,166,.16), 변화 칩 상승 #1E5A3E/#DDF0E5 · 하락 #9A3F33/#FBE9E3,
  영역별 비교 종합 #5FA986 / 문법 #B3A3EA / 주요구문 #8FB8E3 / 독해어휘 #E9C46A / 히즈어휘 #F4B08A. **새 차트를 만들면 이 표에서 색을 고를 것**(짙은 단색 말풍선 금지 — 원장 «우중충» 피드백).
- ⚠ **월간 우측 패널은 숨은 카드(`#monthcap-`)의 outerHTML을 복사해 `dangerouslySetInnerHTML`로 넣는다.** 런타임이 보간값을 `<span class="sc-interp">`로 감싸는데,
  이 span이 **SVG `<text>` 안**에 있으면 재파싱 때 HTML 파서가 SVG를 강제로 닫아(foreign content breakout) 그 뒤 눈금·선·점이 전부 사라진다(원장 «그래프가 안 나와»).
  수리: 복사 직전 `sc-interp` span을 순수 글자로 풀어 넣음(`monthlyActiveCard`). **규칙: SVG 안에 `<text>{{ … }}</text>`를 쓰지 말고 HTML div 오버레이로 라벨을 두거나, 복사 경로가 있으면 위 정리를 거칠 것.**
  같이 넣은 안전장치: 복사한 길이와 숨은 카드 길이가 다르면 240ms 뒤 한 번 더 복사(최대 12회) — 카드가 다 그려지기 전 복사된 경우 대비.
- 검사: `comp_probe.py`(두 달치 기록 → 패널 svg 560×166 lines 3·paths 7·circles 8), monthly_e2e ⑥ 회귀 항목.

### 8-9g. 디자인 정밀 검수 → 미리보기 사본 → 승인 2건 적용 (2026-09-06, v33.157)
- **진행 방식(원장 §14 ① 선-위젯-컨펌 준수)**: 12화면 캡처(`scratchpad/design_shots.py`) → 검수 위젯 → «실제 바꾸면 어떻게 되는지»는 **앱 사본(`_preview_<page>.html`)에 패치를 실제 적용해 렌더**(`render_page.py`)한 전/후 슬라이더 뷰어(Artifact «히즈북스 리뉴얼 미리보기»)로 제시.
  패치는 전부 `scratchpad/patch_preview_<page>.py`(SRC DST 인자, count-assert, JSON 검증) — 승인되면 그 스크립트를 실제 파일에 그대로 재생. 원본 미수정·미커밋 원칙.
- **승인·적용된 것(v33.157)**: ①종합 보고서(`#profilecap`): 월별 성장·영역별 막대·출석 리듬·시험 추이 4차트 파스텔 1안, 헤더 «이미지 복사»만 골드 채움·나머지 글자형·닫기 ×,
  카드 프레임 #FAFAF5/1px #E0C97A/16px, 보조 글자 12px, 하락 코랄(#9A3F33)·0 회색, 상담 패널 토글 선택=세이지, 아이 이야기 «기록» 테두리형.
  로직: 리듬 타일 `ry.bg` 3단계 세이지, `lBg` 세이지/코랄, 월별 점 색, `pwStuBg/pwParBg` 세이지. ②학원일지·한눈에: `_bizSeries` 에 `flat/notFlat`(3개월 재원·매출 동일·신규/퇴원 0) → 차트 대신 한 줄,
  «수강료 장부 열기»·«데이터 탭에서 관리» 테두리형+→, 퇴원생 데이터 카드 안, 금액 산세리프 26px tabular.
- **미승인 대기(사본만 존재)**: 홈·일간·월간·성적·선생님일지·출결·장부 7개 — 뷰어에서 골라 «적용» 지시 시 같은 절차. 사본 `_preview_*.html` 은 untracked(커밋 금지).
- ⚠ 사용량 한도로 워크플로가 끊기면 **에이전트가 남긴 `_pv_t_*`·`chk_*` 류 스크래치가 저장소 루트에 남는다** — 커밋 전 `git status` 로 정리. 이전 세션 파일(`_dbg.py` 등)은 남의 것이니 지우지 말 것.

### 8-9h. 일간 캡처카드 출결 상자 + TODAY 등하원 통합 (2026-09-06, v33.158, 원장 1안)
- `#cap-` 상단 «출결» 상자가 이제 «출결 · TODAY 라벨 · 출결 배지» 행 + 등하원 타임라인(`s.ck.tl`: ● 등원 HH:MM ─ 하원 HH:MM ●) 또는 문장(`s.ck.hasMsg`: «병원 진료 — 미리 연락 주신 결석이에요»)을 품는다.
  하단 hairline «TODAY» 스트립은 제거(같은 rv `s.ck` 를 그대로 소비 — `_ckReport` 무변경). `s.ck.att` 필(«결석 예정» 라벨)은 상자 배지(`s.attendance`)로 대체, 사유·예정 여부는 문장이 담당.
- 검증: `scratchpad/cap_states.py`(4상태 미리보기 모달 캡처 + DOM: TODAY 1회·스트립 없음) + daily_e2e 18 + 게이트 6종. ⚠ 하니스에서 «출석» 기록이 «지각»으로 보이는 건 수업 16:40 시작·등원 16:50 이라 앱의 정시 지각 판정(v33.068)이 맞게 작동한 것(버그 아님).

### 8-9i. 일간 캡처카드 «RECENT N» 리듬 줄 (2026-09-06, v33.159, 원장 1안·수업일 기준)
- 옛 TODAY 자리(시험 성적 아래)에 헤어라인 한 줄: **최근 5회 수업일 점**(출석 채움 초록 · 지각 허니 · 결석 코랄 테두리 · 보강 파랑 테두리 · 오늘 링) + **연속 출석 N회** + **다음 수업 «M.D(요일) HH:MM(보강)»**(v33.162: 원장 지시로 «목 9/10» → «9.10(목)» 형식, 글자 12→13px).
- **v33.206 다음 수업 칩 A안(원장 승인 2026-09-16)**: 작은 라벨 «다음 수업»(11px #9a927e) + 크림 칩(#FFFDF6 · 1.5px #DCBF73 · 999px) 안에 날짜·시간 14.5px 800 딥그린 tabular-nums, **보강일 때만** 금색 «보강» 배지(#C9A227 바탕 · #FFFDF6 글자). `_wkReport` 가 `nD`(날짜)·`nT`(시간)·`nHasT`·`nMk`(학생 보강 or 반 보강 이동)·`nPad` 를 따로 돌려준다(`next` 문자열도 유지). 줄 높이 53px 그대로(v33.205 실측 동일). 검증 `next206_e2e.py`.
  ⚠ 요일(월~일) 기준이 아니라 **수업일 기준** — 주 2회 반에서 빈 점이 생기는 첫 안을 원장이 반려함.
- rv `s.wk = _wkReport(data, classId, sid, name, date)`: 기록은 `_stuOwnRecs`(전 반 통합·id 중복 게이트), 날짜별 상태 = 기록 attendance → `autoAttendance` 폴백, 연속 출석 = 전체 기록에서 최신부터 «결석» 전까지 날짜 수(5회를 넘어도 이어짐),
...
### 8-9l. 장학마일리지 스프라우트 엠블럼 0점부터 (2026-09-07, v33.163, 원장 지시)
- `tierInfo(total)` 첫 가드 «1점 미만 없음» → «null/NaN/음수만 없음». 표의 스프라우트 임계값은 원래 0이었으나 가드 때문에 0.2~0.8점 학생은 막대만 뜨고 엠블럼이 숨었음. 이제 0점(첫 수업 결석 포함)부터 스프라우트 엠블럼+라벨 표시. 음수(과제 미완 누적)는 여전히 숨김.
- 검증 `scratchpad/tier0_check.py`: tierInfo(−0.2/0/0.6/1/10/NaN/null) = null/스프라우트/스프라우트/스프라우트/루키/null/null + 0점 학생 카드 실렌더 엠블럼·«스프라우트» 라벨 존재.

### 8-9m. 장학마일리지 «보강» 0점 규칙 — 원장 확정 (2026-09-07, 코드 변경 없음)
- `mileageDetail(r)` 첫 줄: 출결이 «결석» 또는 «보강»이면 출결 0 · 과제 0 · 시험 0. 마일리지의 유일한 기록별 출처이며(`studentMileage` = startMileage + Σ mileageOf), 다른 경로는 없음.
- 원장 결정: **어떤 보강이든(빠진 수업을 채운 보강·시험 대비 추가 수업 모두) 마일리지 적립 없음.** «보강도 출석과 동일 적립» 제안은 반려됨 — 다시 제안하지 말 것. 보강은 «함께한 N번째 수업» 횟수·출석률에는 포함(변화 없음).

### 8-9n. 일간 성장일지 감성 한 줄 풀 확대 188 → 300편 (2026-09-07, v33.167, 원장 지시 «더 정교하게·감동적·통찰·갯수 확대»)
- `_cmtLines()` 300편. 기존 188편 중 39편은 그대로, 나머지는 결을 유지한 채 한 호흡 더 붙여 다듬음(예: «오늘 안 되면 내일 하면 돼. 그건 포기가 아니라 호흡이야.»). 새 결 **G 말과 언어·질문** 20편 추가(영어학원 정체성: «새 언어를 하나 배운다는 건 세상을 하나 더 갖는 거야.»).
- 결 7개: A 자라는 일과 시간 50 · B 실패·두려움·용기 50 · C 너 자신 50 · D 배우는 태도 50 · E 사람과 세상 40 · F 오늘과 삶 40 · G 말과 언어 20. 길이 18~43자(카드 18px에서 2줄 이내).
...
### 8-9q. «학생 개별 보강» 복수 선택 (2026-09-09, v33.170, 원장 지시)
- 보강 허브 → «학생 개별 보강» 선택 화면이 **여러 명 토글 선택**으로 바뀜. 고른 행은 체크(✓)+남색 테두리+연보라 바탕, 하단 고정 바에 «선택 해제»와 «N명 보강 예약 →». 상태 `S.mkSel = {sid:{cid,name}}`.
  ⚠ 예전엔 학생을 누르는 즉시 모달이 열렸다. 이제 누르면 **선택만** 되고, 하단 버튼으로 진행한다.
- `S.mk` 에 **`list:[{sid,cid,name}]`** 도입. 단일 진입점 6곳(`openStuMakeup`: 달력 이벤트·다가오는 보강·학생관리 «보강»·관제판 결석·안전망 목록)은 **손대지 않았고** list 길이 1로 같은 코드 경로를 탄다.
  다중이면 제목 «N명 · 개별 보강 예약», 모달 상단에 이름 줄, 완료 화면은 **학생별 «안내문 복사» 행**(한 부모에게 다른 학생 이름이 가지 않게 안내문을 학생별로 생성). 단일이면 기존 화면 그대로(본문 한 상자 + 복사 버튼 1개).
- 기록은 `setStudentMakeupMany(items, date, start, end, fromDate)` — **한 번의 setState·persist**로 N명 `dd.mkup[sid|date]` 작성, 활동 로그(`_actEntry`)는 학생별로 남긴다. 시간 자동 채움은 **첫 학생의 반 시간표** 기준(`_mkSet('to')` 기존 로직).
  ⚠ 다중일 때는 «이미 잡힌 보강» 확인(`mkHasCur`)을 건너뛰고 항상 예약 폼을 연다(단일은 종전대로 보강 취소 버튼).
...
### 8-9s. 보강 종류 구분 — «결석 보강» / «추가 보강» (2026-09-09, v33.172, 원장 지적)
**증상**: 홈 «오늘 보강» 배지가 **모든 개별 보강을 «M월 D일(요일) 결석 보강»으로** 적었다. 결석과 무관하게 «부족한 부분을 채우는 추가 보강»도 있는데, 그 경우 날짜(= 예약한 날)와 «결석»이 둘 다 사실이 아니었다.
**원인**: `dd.mkup[sid|date]` 레코드에 **종류가 저장되지 않았다**. 모달 상태 `S.mk.free`(1 = 개별/추가)는 있었지만 저장 때 버려졌고, 표시하는 쪽은 `from` 이 항상 채워져 있어 무조건 «결석 보강»으로 찍었다.
**수리**:
- 레코드에 **`free`** 저장(`setStudentMakeupMany(..., free)`). 진입점이 기본값을 정한다 — 허브 «학생 개별 보강»·복수 선택 = **추가 보강**, 결석 학생 «보강 잡기»·관제판 = **결석 보강**.
- 예약 모달에 **종류 선택 버튼 2개**(결석 보강 / 추가 보강)를 넣어 그 자리에서 바꿀 수 있게 함(`_mkSet('free', 0|1)`).
- 판정 정본 **`_mkupKindFree(dd, m, sid)`** — `m.free` 가 있으면 그것, **없으면(v33.171 이전 기록) `from` 날짜의 출결로 추정**(결석·보강이면 결석 보강, 아니면 추가 보강).
- 표시: 홈 배지는 추가 보강이면 **날짜 없이 «추가 보강»**(예약일은 의미가 없다), 결석 보강이면 «M월 D일(요일) 결석 보강». 학부모 안내문 «추가 보강 수업» / «결석 보강 수업». 활동 로그도 분기(추가 보강은 «결석 →» 구간 없음).
- `_mkupList` 가 `free` 를 함께 넘기고, `openStuMakeup` 은 인자 `free` 가 없으면 저장된 종류를 복원한다(홈 배지·달력·허브에서 다시 열 때).
- 검증 `scratchpad/mkkind_e2e.py` **15항목**(허브=추가·결석경로=결석·모달 전환·안내문 문구·홈 배지 표기·다시 열기 복원·예전 기록 추정 2종) + 게이트 6종 + daily/mkmulti/namesort E2E.

### 8-9t. 보강 «일정 수정» (2026-09-09, v33.173, 원장 지시)
- 잡힌 보강을 열면 종전엔 «…에 잡혀 있어요» + [보강 취소] 뿐이라 **취소하고 다시 잡아야** 날짜·시간을 바꿀 수 있었다. 이제 [**일정 수정**] [보강 취소] 두 버튼.
- `mkOnEdit` 가 현재 기록(날짜·시간·종류)을 폼에 채우고 `S.mk.edit=1`, **원래 날짜를 `origTo` 에 기억**. `mkHasCur` 는 `!S.mk.edit`, `mkPick` 은 `S.mk.edit ||` 로 폼을 연다. 확정 버튼은 «수정 저장», 완료 문구는 «보강 일정을 수정했습니다».
- ⚠ **키가 `sid|날짜` 라 날짜를 바꾸면 중복이 생긴다.** `mkConfirm` 에서 `m.edit && m.origTo !== to` 이면 **먼저 `clearStudentMakeup(sid, origTo)`(삭제 표식)** 후 새 날짜에 쓴다. E2E 로 «살아있는 보강 1건» 확인.
- 수정 중에도 종류(결석/추가) 버튼이 그대로 보여 함께 바꿀 수 있고, 학부모 안내문도 바뀐 종류로 다시 만들어진다. 현재 보강 안내 문구에 종류를 붙였다(«추가 보강이 9월 15일(화) …에 잡혀 있어요»).
- 검증 `scratchpad/mkedit_e2e.py` **16항목**(버튼 존재·폼 채움·시간만 변경·날짜 변경 시 옛 기록 삭제·중복 없음·종류 변경·안내문·다시 열기·보강 취소 회귀·신규 예약 버튼 문구) + 게이트 6종 + daily/mkkind/mkmulti E2E.

### 8-9u. «결석 예정 체크 · 안 온 학생» 에 보강 등원 학생 추가 (2026-09-09, v33.174, 원장 지시)
**증상**: 이 패널은 «그 날 수업이 있는 반»만 훑는다(`_classStart` 없으면 return). 그래서 **그날 보강으로만 오는 학생은 목록에 아예 없어** 결석 예정 체크도, 보강 확인도 못 했다.
**수리**: 반 순회가 끝난 뒤 `_mkupList(data, date)` 를 훑어 **«보강 등원» 그룹**으로 붙인다.
- 담당 선생님 열 아래에 반 이름 대신 «보강 등원» 그룹을 만들고, 칩에 종류+시간을 표기(«추가 보강 20:00 ~ 21:30» / «결석 보강 16:00 ~ 18:00», `_mkupKindFree` 사용). 칩 배경은 보강 색(#eef0fa/#c5cae9)으로 구분.
- **중복 방지 `_has9`** — 그날 수업도 있고 보강도 있는 학생은 한 번만(정규 칩 우선).
- 이미 등원(`checkins[sid|date].in`)했으면 제외, 총원(`count`)에도 포함, «결석 예정» 토글도 그대로 동작(키가 `sid|date` 라 그대로 쓰인다).
- 보이는 반(`visibleClasses`) 안에서 학생을 찾지 못하면 건너뛴다 → 담임 스코프 유지.
- 템플릿: 보강 버튼 게이트를 `{{ s.isExc }}` → **`{{ s.showMk }}`** 로 바꿈(정규 칩은 종전대로 결석 예정일 때만, 보강 등원 칩은 항상 «보강 확인»).
- 검증 `scratchpad/excmk_e2e.py` **14항목**(추가/결석 보강 2명 표시·담당 선생님 아래 배치·중복 없음·총원 합산·버튼 노출 규칙·결석 예정 처리·등원하면 제외·보강 없는 날짜는 종전과 동일) + 게이트 6종 + daily/mkedit/mkkind E2E.
- ⚠ **남은 것**: 위쪽 «오늘 등하원 관제»(수업 있는 반만 표시)는 아직 보강 등원 학생을 안 보여준다 — 원장 요청 범위 밖이라 두었다.

### 8-9v. «오늘 등하원 관제» 보강 등원 줄 + 일간일지 «오늘 보강» 배지 (2026-09-09, v33.175~176, 원장 지시)
**관제판(v33.175)**: `adminCk` 는 «그 날 수업이 있는 반»만 줄을 만들어(`_classStart` 없으면 return) 보강으로만 오는 학생의 등원·하원을 추적할 수 없었다.
- 반 순회 뒤 `_mkupList(data, date)` 로 «반이름 · 추가 보강 / 결석 보강» 줄을 추가. 시간은 **보강 시간**(`m.start~m.end`) 기준으로 등원/하원/하원 미기록/결석 예정/미등원/보강 대기를 판정하고, 상단 카운터(kTot·kIn·kOut·kMiss·kExc·kUnrec)에도 합산. 줄은 `start` 기준 정렬이라 보강 시간대로 자연히 끼어든다.
- 중복 방지 `_sn9`(반 줄에 이미 있으면 건너뜀), 담당 선생님 필터(`schedTeacher`)·퇴원·접수대기·시작 전 제외는 반 줄과 동일. 안내 문구 «수업 있는 반만 표시» → «수업 있는 반 · 보강 등원».
- ⚠ **his-check R1 신규 4건**이 났다(내 `_pool9` 2건 + 기존 관제 줄이 지문 변경으로 2건). 관제판은 원래부터 학원 전체 반이 대상이므로 `ALLOW['_ckT']` 에 사유를 등록해 예외 처리(→ 0건). **전 반을 훑는 화면은 ALLOW 에 «왜»를 남길 것.**
**일간일지(v33.176)**: 원장 선택 = «카드에 표시»(명단·저장 경로는 손대지 않는 안전한 쪽).
- **v33.177 자리 정정(원장)**: 카드 머리(딥그린 헤더)의 배지는 **반려** — «원래 자리», 즉 **등원 상태 줄**(«● 아직 등원 전 · 수업 시작 18:30 · 수동 등원 기록»)에 넣을 것.
  - `ck` 빌더가 그날 보강 문구 `_mkLb8` 를 만들어 각 상태(등원/결석 예정/아직 등원 전)에 `mk` 필드로 실어 보내고, 상태 줄에 남색 알약으로 그린다.
  - ⚠ 그 반이 **그날 수업이 없으면 상태 줄 자체가 안 뜬다**(`_classStart` 없으면 `{show:false}`). 그래서 보강이 있으면 줄을 띄우도록 `_mkFb8()` 폴백을 넣었다 — «● 아직 등원 전 · 추가 보강 20:00 ~ 21:30» + **수동 등원 기록 가능**. 휴원일 분기에도 같은 폴백.
  - v33.176 의 `mkToday`(머리 배지)는 렌더값만 남고 템플릿에서 제거됨.
- ⚠ **반려된 대안**: «다른 반 보강 학생도 명단에 합류» — `saveBulk` 가 `classId: s.activeClassId` 로 저장해서 기록이 엉뚱한 반에 붙는다(월간·성적·마일리지 집계가 어긋남). 하려면 행마다 cid 를 들고 다니도록 저장 경로를 고쳐야 한다.
- 확인: 일간일지는 **반을 고르면 그 반 학생이 오늘 수업 없는 날에도 이미 다 나온다**(`bulkRows` 에 날짜 필터 없음). 그래서 «보강 학생이 안 보인다»는 다른 반일 때만 생긴다.
- 검증 `scratchpad/ckboard_mk_e2e.py` **16항목**(두 종류 줄·담당 표기·중복 없음·총원 합산·시간순·등원/하원/결석 예정 반영·보강 없으면 줄 없음) + `bulkmk_e2e.py` **7항목**(두 종류 배지·화면 표시·보강 있는 학생만·다른 날짜·보강 취소 시 사라짐) + 게이트 6종 + daily/excmk/namesort E2E.

### 8-9w. 보강일 출결 자동 «보강» (2026-09-10, v33.178, 원장 지시)
**숨어 있던 구멍**: §8-9m 에서 «모든 보강은 마일리지 적립 없음»으로 확정했지만, 그건 출결을 **«보강»으로 저장했을 때만** 성립했다. 실제 보강 날에는 `autoAttendance` 가 등원 기록을 보고 **«출석»(+0.2)** 을 기본값으로 넣어, 보강에도 마일리지가 붙고 있었다.
**수리**: `autoAttendance` 첫머리(시작 전 학생 가드 다음)에 `if(this._mkupOf(data, sid, date)) return '보강';` — **등원 기록보다 먼저** 판정한다. 그 아래에 있던 «보강이면 null» 분기는 닿지 않으므로 제거.
- 결과: 보강 잡힌 날 일간일지 출결이 자동으로 «보강»(마일리지 0), 등원 기록이 있어도 «출석»으로 뒤집히지 않는다. 카드 상태 줄의 «추가 보강 20:00 ~ 21:30» 배지(§8-9v)와 짝이 맞는다.
- 선생님이 손으로 고른 값은 그대로 — `loadDraft` 는 저장된 기록의 `attendance` 를 먼저 쓰고, 비어 있을 때만 `autoAttendance` 로 채운다. 보강을 취소하면 종전 판정(등원 시각 기준 출석·지각)으로 돌아간다.
- **결석 보강의 원래 결석일은 영향 없음** — mkup 키가 `sid|보강일` 이라 결석일에는 `_mkupOf` 가 안 걸린다.
- 검증 `scratchpad/autoatt_mk_e2e.py` **10항목**(자동 보강·마일리지 0·등원해도 유지·수업 없는 반·손으로 고른 값 유지·보강 취소 복귀·결석일 유지) + 게이트 6종 + daily/monthly/ckboard/bulkmk/cap_week.

### 8-9x. «내 메모 · 할 일» 카드 (2026-09-10, v33.179, 원장 지시)
- 선생님 일지·학원 일지의 **월간 일정 카드 바로 아래**(같은 열)에 개인 메모 체크리스트. 템플릿 3벌(달력 블록이 3벌 복제돼 있음) 모두 같은 마크업 — R4(복제 화면 불일치) 대비.
- **로그인한 사람 것만 보인다**(«나만 보여요»). 데이터 `data.memos = { 사용자: [ {id,text,done,t,del} ] }`. 안 끝난 것 최신순 위 → 끝낸 것 아래(최근 30개), 머리글에 «N개 남음 / 모두 끝냈어요».
- 적기(Enter 또는 «추가», 공백만이면 무시, 300자 제한) · 체크 토글 · × 삭제. **삭제는 `del:1` 표식**(§8-10 교훈 — 표식 없이 지우면 다른 기기 사본이 되살린다).
- ⚠ **`mergeAppData` 규칙 필수**: `dd.<키>` 를 쓰면서 합치기에 없으면 his-check **R3** 가 «두 기기가 동시에 쓰면 한쪽이 통째로 사라짐»으로 잡는다. `out.memos = _hisMapMerge(..., (b,o)=>_hisArrMerge(b,o, x=>'mm:'+x.id, 최신 t 우선))` 을 bdayCele 옆에 추가했다.
- 카드에 `data-memocard="1"` 표식 — 하니스가 카드를 정확히 집을 수 있게(이 표식이 없으면 innerText 로 찾다가 조상 div 를 잡는다).
- 검증 `scratchpad/memo_e2e.py` **16항목**(빈 상태·Enter 추가·입력칸 비움·추가 버튼·공백 무시·체크 정렬·개인 스코프·삭제 표식·합치기 규칙 정적 확인·학원 일지에도 표시) + 게이트 6종 + daily/monthly E2E.
- 원장 판단 대기: 지금은 **개인 메모**다. «선생님들끼리 공유하는 게시판»이 필요하면 별도 — 이미 «보고·건의함»·«원장님 전달 사항»이 그 자리에 있다.

### 8-9y. 일간일지 머리 줄 재배치 (2026-09-12, v33.180, 원장 지시)
- **시험 D-day 배지**를 오른쪽 작업 묶음(`bulk-actions`)에서 빼내 **왼쪽 «등원 N/M» 배지 바로 오른쪽**으로. 머리 줄이 «상태(반·날짜·등원·시험) | 작업(도구·점수·저장·전송)» 두 묶음으로 갈린다.
- **«점수 일괄 입력»**을 ⋯도구 드롭다운에서 꺼내 **«1 저장» 앞 독립 버튼**으로. 스타일은 도구와 같은 테두리형 — 채움(딥그린)은 «1 저장» 하나만 유지(DESIGN.md 규칙). 도구 메뉴에는 오답 시험 출제·시험지 스캔 채점·장학마일리지 안내만 남음.
- ⚠ **D-day 배지 표시 규칙 (v33.180 작업 중 내가 오해했던 것 — 원장 정정)**: `_examCalc` 의 `const armed = !!v.on; if (!armed && !force && n > 20) return null;`
  · **활성(`on:true`, 시험 설정의 활성 버튼) → 기한 무관 항상 표시.** 실측: D-31·D-120 모두 뜸. 원장이 «언제든 입력하고 활성 누르면 바로 생성»으로 설정해 둔 그대로다.
  · 비활성(`on` 없음) → **D-20 이내에만** 자동 표시(D-21은 안 뜸, D-20은 뜸).
  ⚠ 하니스에서 `{label,date,t}` 만 시드하고 먼 날짜를 쓰면 배지가 없어 «기능이 안 된다»로 오판한다 — 먼 날짜를 검사하려면 `on:true` 를 같이 넣을 것. 나는 D-31·비활성으로 시드해 놓고 «30일 창» 이라고 원장에게 잘못 보고했다.
  학교 키는 `_schoolKey(_stuSchool(stu))` 로 만들어야 `examSchool` 과 맞는다(«포항동지고 1학년» → «포항동지고»).
- 검증 `scratchpad/hdrbar_e2e.py` **13항목**(가로 좌표로 순서 확인: 등원 < D-day < 도구 < 점수 일괄 입력 < 저장 / 드롭다운에서 제거 / 나머지 도구 항목 유지 / 버튼이 실제로 열림 / 시험 없으면 배지 없음) + 게이트 6종 + daily/memo E2E.
...
**⛔ v33.202 에서 소리 엔진 전체 삭제**(원장 승인 D3, 2026-09-15). 차임이 기본 꺼짐으로 종결된 뒤에도 엔진(`_tick`·`_tickPlay`·`_newAC`·`_ensureKeep`·`_adiag`, 약 9KB)과
진단 기록(`data.audioDiag` 클라우드 동기화 · 데이터 화면 «키오스크 소리 진단» 카드 · 잠금 화면 좌하단 `#kio-adbg`) · «출석 확인음(차임벨)» 스위치가 남아
키를 누를 때마다 오디오를 깨우고 진단을 기록하고 있었다. 전부 제거하고 `mergeAppData` 가 `audioDiag`·`kioskChime(T)` 를 버린다(모든 기기 업데이트 후 클라우드에서도 사라짐).
확인은 성공 화면(이름·시각)이 담당. 키패드 즉시 반응(pointerdown·`_tsG` 가드·`_kFast` 점 칠하기)·화면 유지(Wake Lock)·새벽 자동 새로고침은 그대로.
검증 `rm202_e2e.py`(진짜 키 누름: 등원·틀린 번호·하원·점 1개·PIN 나가기 · 오디오 엔진 생성 0회). ⚠️ 첫 등원은 세리머니라 성공 화면이 3.5초 — 연속 입력 테스트는 4초 이상 기다릴 것.
아래는 기록용.
...
**v33.205 남은 구멍 (원장 신고 2026-09-15: 21:00 수업 · 21:02 등원인데 «출석»)**: v33.068 은 «저장된 기록»만 고쳤다. **저장 전 임시입력 행**은 그대로였다 —
등원 전에 카드를 한 번이라도 건드리면(한마디·과제·점수) `setBulk` 가 loadDraft 기본값 «출석»(등원 전이라 자동 판정이 null)을 통째로 `bulk.rows` 에 복사하고,
이후 화면·저장이 그 행만 읽어 늦은 등원이 반영되지 않았다(재현: 등원 20:58/수업 20:56 → 화면·저장 모두 «출석»).
수리: loadDraft 새 임시입력에 `attAuto:1` → `setBulk('attendance')` 가 0 으로(손선택) → `_attFresh(data,cid,sid,date,row)` 가 attAuto 행이면 `autoAttendance` 를 다시 적용
(**출석·지각·보강만** — 등원 없는 «결석»으로는 자동 전환 안 함). 적용처 = bulkRows 카드 · getDraftBulk · 점수 일괄 입력 표 · saveBulk 2곳(저장 기록에서 attAuto 제거).
⚠️ 이 버전 이전에 만들어진 임시입력 행(attAuto 없음)은 손선택으로 취급돼 자동 갱신되지 않는다 — 그날 카드는 «지각»을 한 번 눌러야 한다.
⚠️ 남은 것(원장 판단 대기): 카드를 건드린 뒤 **끝내 안 온** 학생은 여전히 «출석»으로 남는다(«결석» 자동 전환은 과제·시험 연쇄 규칙과 얽혀 이번에 넣지 않음).
검증 `late205_e2e.py`(늦은 등원 → 화면·저장 «지각» / 제때 등원 «출석» / 손으로 고른 «출석» 유지).
...
### 8-23. 성능 1단계 (2026-09-14, v33.181~v33.185, 원장 지시 «앱 실행이 전반적으로 느려 문제 파악하고 … 속도 향상»)
**진단(실측, 제 PC 크로미움, 네트워크 제외)**: 느린 원인은 데이터 양이 아니라 구조 — ① 부팅 때 원본 화면 설계도(`<x-dc>` 안 요소 5,900개·base64 이미지 29장)를 브라우저가
런타임 컴파일 전에 통째로 레이아웃(1.9~2.3초) ② 글자 1타마다 renderVals(841개 값)+템플릿 전체 재평가 ③ 저장 뒤 1분간 1.2초마다 툴팁 글자 때문에 전체 재그리기
④ 학생마다 전체 기록을 훑는 곳(케어 힌트·이탈 위험·요약·미입력) ⑤ 저장·수정 160곳이 데이터 전체를 JSON 깊은 복사(3.6MB 기준 100ms) + persist 마다 저장소 재파싱.
기록이 3.7배 늘면 타이핑도 3.7배 느려지는 선형 구조였다(3,114건 45ms → 11,466건 165ms).

**적용(한 항목 = 한 버전, 매 버전 게이트 6종 + 전용 E2E)**
| 버전 | 내용 | 전후 |
|---|---|---|
| v33.181 ① | `<head>`에 `x-dc>:not(#his-splash){display:none!important}` 1줄 — 런타임이 x-dc 를 #dc-root 로 바꾸면 더는 안 걸림. splash 는 보임 | 앱 준비 3.3~4.4초 → 1.4초 (5회 반복) |
| v33.182 ② | 타이핑 오버레이 `_typ/_typGet/_typSet/_typFlush` — 입력 중엔 setState 안 하고 0.26초 멈춘 뒤 한 번. renderVals 는 오버레이를 상태보다 먼저 읽음. focusout(캡처)·beforeunload·saveBulk·commitRecord·openBulk 에서 즉시 반영. 대상: 일간 진도/과제/한마디·시험 점수/만점(b.onScore/onMax)·점수 일괄 표·아이 얘기 한 줄·월간 종합 평가·시험 메모(+v33.183 과제 항목 이름) | 1타 45ms→2ms(1MB), 165ms→4ms(3.6MB) |
| v33.183 ③ | homeData.todaySchedule(템플릿 미사용) 계산 생략 · 저장 표시 타이머는 보이는 글자/점 색이 바뀔 때만 setState, 툴팁은 `[data-sync-full]` 에 직접 · `_resizeMonthlyComments` 는 글자·창 너비가 같은 칸 건너뜀 | 저장 뒤 1분간 매 1.2초 전체 재그리기 0회 |
| v33.184 ④ | `_ri` 색인에 `cc`(반)·`cd`(반|날짜) 추가, `_recsC/_recsCD` — 그리기 경로 원시 훑기 24곳을 색인 조회로. 12개 화면 렌더값 JSON(552키) 두 버전 완전 동일 확인 | renderVals 1회: 일간 42→22ms · 선생님 일지 63→35ms · 학원 일지 24→12ms (기록 1.1만) |
| v33.185 ⑤ | `__hisCloneData` — records/exams 는 배열 새로+항목 얕은 복사, 나머지는 깊은 복사. 160곳 치환. persist 는 저장소 문자열이 우리가 마지막에 쓴 그대로면 재파싱 생략(다른 탭이 썼으면 예전처럼 파싱) | 복사 1회 55~60ms → 17ms(기록 1.1만) · 연속 저장 시 저장소 재파싱 0회 · 기록·반·항목 모두 새 객체(격리 E2E) · 12개 화면 렌더값 동일 |

⚠️ **React 제어 입력칸의 함정(②)**: 이벤트 뒤 React 가 `value` 를 마지막 렌더값으로 되돌린다. 오버레이만으로는 글자가 한 자씩 사라진다 — 요소의 `__reactProps$…​.value` 를 타이핑값으로 맞춰 되돌림을 무동작으로 만든다(DOM 은 안 건드려 한글 조합 안전). props 를 못 찾으면 예전 방식(즉시 setState)으로 폴백. 프로그램이 값을 바꾸는 setBulk/setExam 은 해당 키의 오버레이를 버린다(자동 번호·추천 코멘트·일괄 채움이 이김).
⚠️ **`__hisCloneData` 는 window 에도 노출** — 앱 스크립트는 `new Function` 안에서 돌아 전역이 아니다. 기록 항목 안의 중첩 객체(hwCheck 배열 등)는 공유되므로 제자리에서 바꾸지 말고 새 객체로 교체할 것(현재 코드는 모두 교체 방식).
⚠️ **줄 단위 프로파일(positionTicks)은 인라인된 클로저의 시간을 바깥 함수 시작 줄에 몰아 붙인다** — «todaySchedule 26%» 는 실제로 homeData IIFE 전체였다. 메서드 단위 계측(`bench_methods.py`)이 정확하다.
...
| v33.188 2-3 | **⛔ v33.201 에서 삭제(원장 승인 D2 — 기록 1,212건·보관함 0건, 없어도 되는 기능이었음). 아래는 기록용.** **기록 보관함(A안, 원장 확정 2026-09-14)** — 경계 `data.archiveCut`('YYYY.MM.DD', 기본 제안 2026.03.01)+`archiveCutT`(합치기: 도장 최신 우선). **메모리 `state.data.records` 는 전체 기록 그대로 → 계산·화면 100% 동일**(5개 화면 렌더값 옮기기 전후 동일 검증). `persist` 가 경계 이전 기록을 `his-arch-v1` {records} 로, 나머지만 `his-sys-v4` 로 나눠 씀(보관함 문자열이 바뀐 때만). 생성자·`_absorbFresh` 는 본 데이터+보관함을 키 `classId|studentId|date`·editT 최신으로 합침. 동기화: `app_state` 행 `archive` — 보관함 키가 바뀌면 2초 뒤 병합 후 업로드(`schedulePushArch`), 부팅·2분마다 `updated_at` 확인 후 내려받아 합침(`pullArchive`) → `his-archive-refresh` 로 앱 흡수. `window.__hisArch{pull,push,merge}`·`window.__hisMergeAppData` 노출(검증용). 데이터 화면 «기록 보관함» 카드: 날짜 + «이 날짜 이전 기록을 보관함으로» / «모두 되돌리기»(경계 ''=전부 본 데이터) | 경계 이후 저장·전송·부팅 파싱 크기가 현재 기록 분량만큼으로 |

⚠️ **보관함 원칙**: 기록은 삭제되지 않는다(삭제 톰스톤 없음) → 보관함은 «합집합 + editT 최신» 만으로 안전. 경계를 바꾸면 분할이 결정적으로 다시 계산되므로 «옮기기/되돌리기»는 경계값 변경일 뿐이다. 옛 앱 버전 기기가 전체 기록을 본 행에 올려도 새 버전이 합친 뒤 다시 나눠 쓰므로 유실 없음(잠깐 무거워질 뿐). 보관함을 아직 못 받은 기기(`__arch_at` 없음)에서 저장하면 persist 가 빈 보관함을 쓰지 않도록 `_archLoaded` 로 막고, push 는 항상 클라우드 행과 합친 뒤 올린다.
⚠️ 원장이 카드에서 «옮기기»를 누르기 전까지는 아무것도 달라지지 않는다(경계 ''). 첫 실행 후 다른 기기는 2분 폴링에서 보관함을 받는다.

**3단계 (2026-09-14, v33.191, 원장 지시 «3단계 진행») — 클라우드 «바뀐 부분만 전송»** — **⛔ v33.201 에서 삭제**(원장 승인 D1). 데이터 1.9MB라 이득이 거의 없었고, 켜면 알림톡 서버가 읽는 `main` 행에서 등하원 기록이 빠져 **이미 등원한 학생에게도 미등원 알림이 나가는 구멍**이 있었다(서버 판정 코드 시뮬레이션: 6명 오발송). 한 번도 켜진 적 없어 데이터 영향 없음. ⚠️ 다시 만들 일이 생기면 서버가 `main` 만 읽는다는 사실부터 확인할 것. 아래는 기록용.
- **원칙**: 기기 저장소(`his-sys-v4`)는 지금처럼 전체 한 벌(자동 백업 `snap_*`도 그대로). **클라우드 전송만** 조각으로: `main`(반·학생·설정 등 — records/checkins/exams 제외) · `exams` · `rec_YYYY-MM`(월별 기록, 보관함 이전 것은 archive 행) · `ck_YYYY-MM`(월별 등하원). 모두 같은 `app_state` 표의 행(추가 표·정책 없음, 공개 키만).
- **전환**: `data.syncMode` 'parts'|'' + `syncModeT`(합치기: 도장 최신 우선). 데이터 화면 «클라우드 전송 방식» 카드 스위치. **기본은 지금 방식(전체 전송)** — 원장이 켤 때까지 아무것도 바뀌지 않는다. 켤 때 안내: 모든 기기가 최신 버전으로 업데이트된 뒤(옛 기기는 15분 안에 자동 갱신).
- **올릴 때** `flushPushParts`: 조각 JSON 해시(`__his_pushed`)가 마지막 올린 것과 다른 조각만 → 그중 클라우드 `updated_at`이 내가 본 값(`__his_parts`)과 다른 조각은 내려받아 `_mergeRow`(mergeAppData 부분 적용)로 합친 뒤 upsert. 이번 달 기록 하나 고치면 그 달 조각 하나(전체의 약 10%)만 올라간다.
- **내려받을 때** `pullPartsMerged`: `select id,updated_at`을 `or(id.eq.main,id.eq.exams,id.like.rec_*,id.like.ck_*)` 한 번으로 조회 → 바뀐 조각만 `in()`으로 받아 그 달/영역만 합침(main은 heavy 키 제거 후 합침) → 기기 저장소 → `his-silent-refresh`(12초 폴링 `silentKioskRefresh`는 parts 모드면 이 경로, 등원 토스트는 그대로). 부팅은 main 행 처리 뒤 `pullPartsApply`.
- **검증**: `sync_e2e.py`(가짜 app_state 클라이언트 `in/or/eq.maybeSingle/upsert/update`) 15항목 — 조각 합=전체, 첫 올리기 전 조각, 무변경 0회, 한 달만 변경 시 1조각, 병합 후 올리기(양쪽 보존), 바뀐 조각만 내려받기·앱 반영, 무변경 재수신 없음, 합치기 규칙, 되돌리기. `window.__hisSync{push,pull,pullMerged,split,mode,hash}` 노출.
⚠️ **옛 버전 기기와의 공존**: parts 모드가 켜지면 클라우드 main 에서 기록이 빠진다. 옛 버전 기기는 자기 기록을 main 에 통째로 올리지만(새 기기는 main 의 heavy 키를 버림) 새 기기가 rec_ 행에 올린 기록은 옛 기기가 업데이트될 때까지 못 본다(유실 아님·지연). 그래서 스위치는 «모든 기기 최신» 뒤에.
⚠️ 클라우드 반 이름은 기존 규칙대로 «내 사본이 이김»(`mergeAppData` classes: over.name 우선) — 부분 전송도 같은 규칙을 그대로 쓴다. 실계정 없이 검증할 수 없는 부분(실제 PostgREST `or`/`in` 응답 형태)은 supabase-js v2 문법대로 작성했고, 켠 뒤 첫 저장·첫 폴링을 원장 화면에서 확인해야 한다.


### 8-24. 상담 질문 칩 — 편집 모드·삭제 도장·실수 방지 (2026-09-14, v33.189, 원장 지시)
**계기**: 원장이 상담 *내용*(200자 넘는 학부모 피드백 두 단락)을 «학부모에게 물을 질문 추가» 칸에 넣어 질문 칩으로 저장됐다. 질문 칩은 `data.counselQ{student,parent}` 문자열 배열이고 **삭제 수단이 없었으며**, 기기 간 합치기가 `_hisUnionStr` 합집합이라 지워도 다른 기기 사본에서 되살아나는 구조였다. 추가 입력칸이 기록칸 바로 위에 항상 열려 있어 헷갈리기 쉬웠다.
**수리**: ① «질문 편집» 버튼(안내 줄 오른쪽) — 켠 동안만 칩이 «✕ 질문»으로 바뀌어 누르면 삭제(confirm), 추가 입력칸(«새 질문 한 줄 (80자 이내)» + «질문 추가»)과 안내(«상담 내용은 여기가 아니라 아래 ‹새 상담 추가›에»)가 나타남. 평소엔 입력칸이 없다. ② 삭제는 `_markDel(dd,'cq',group+'|'+text)`, 추가는 `_markAdd` — `counselQBank()` 가 `delT > addT` 인 질문을 숨긴다(합집합에 남아 있어도 안 보임, 다시 추가하면 addT 가 이김). ③ `addCounselQ` 는 80자 초과·여러 줄을 거부하고 status 로 안내. 화면 3벌(학원 일지 학생 목록 · 일정 · 선생님 일지) 동일 적용(R4).
⚠️ 원장 데이터에 이미 들어간 두 단락은 원장이 «질문 편집 → ✕»로 지운다(내가 원장 데이터에 손댈 수 없음). 지운 뒤 다른 기기와 합쳐져도 되살아나지 않는다.
⚠️ **패턴**: «칩을 누르면 기록칸에 삽입»되는 목록 옆에 자유 입력칸을 상시 노출하지 말 것 — 편집 모드 뒤로 숨기고, 길이·줄 수로 종류를 구분해 잘못 들어온 글을 막는다.

### 8-25. 일간일지 학생 카드 압축 A안 (2026-09-14, v33.190, 원장 승인 — 목업 https://claude.ai/code/artifact/3cc415b8-a9a3-4a5f-8f20-2ff1fcffcc5c)
**지시**: 입력창 세로 폭 축소 · 점수칸 축소 · «저장 후 미리보기» 상단 배치 · «전송 완료» 필요성 재검토 → 현재/A/B 목업을 먼저 보이고 A 승인.
**적용**: ① 출결·과제 한 줄(`.m-attrow`) ② 점수 상자 한 줄형 — 시험명(min-width 36)·스위치 30×17(`togglePos` 15/3px)·점수 30px@17·만점 22px@11, 상자 90→44px. 카드가 640px 보다 좁으면 `@container` 로 2열 ③ «저장 후 미리보기»를 머리 줄 금색 버튼으로, 카드 끝 버튼 줄(`.bulk-btn-row`) 제거 ④ «전송 완료» 버튼 → 머리 줄 «전송 ✓» 표식(`r.isSentOn`, 누르면 `onToggleSent` 로 해제; 켜는 것은 미리보기 창의 «전송 완료 체크») ⑤ 입력칸 시작 높이 72→56/48(자동 확장 유지) · 과제 확인 상자·아이 이야기 줄 여백 축소(«지난 기록은 선생님 일지에서» 문구 제거).
**결과**: 같은 예시 카드 763 → 576px(−25%). **카톡 리포트(캡처카드 `cap-*`) DOM 은 이전 버전과 완전히 동일**(card_e2e ⑨). 태블릿 세로 800px 정상.
⚠️ 한 줄형 점수 상자는 상자 폭 152px 이상이 필요하다 — 폭이 더 줄면 시험명이 0폭이 된다. 폭을 더 줄이려면 `@container` 경계(640px)를 올릴 것. `togglePos` 값은 스위치 폭과 짝이다(30px ↔ 15/3).

### 8-26. 내 메모 · 할 일 타임스탬프 A안 (2026-09-15, v33.192, 원장 승인)
**지시**: 메모에 날짜·시간 타임스탬프 → 목업(A: 메모 아래 작은 줄 / B: 오른쪽 끝 짧게) → A 승인.
**데이터**: 메모 `t` 는 체크·삭제 때마다 바뀌는 **합치기용 최신 시각**이라 작성 시각으로 쓸 수 없다 → 작성 시각 `c`(addMemo), 완료 시각 `dt`(toggleMemo, 풀면 0) 를 새로 둔다. 예전 메모(`c` 없음)는 처음 체크할 때 그 순간의 `t` 를 `c` 로 옮겨 보존. 합치기(`mergeAppData` memos)는 메모 객체를 통째로 고르므로 수정 불필요(memo_ts_e2e ⑤ 양방향 확인).
**표기** `_memoWhen(x)` → 렌더값 `mm.when`: 오늘 «오늘 오후 2:32» · 어제 «어제 오후 6:10» · 올해 «9.10 (목) 오전 11:05» · 작년 이전 «2025.12.3»(시각 생략) · 정오 «오후 12:00»·자정 «오전 12:05». 완료 메모는 «9.8 (화) 오후 3:20 작성 · 9.12 (토) 완료», 작성 시각을 모르면 «9.12 (토) 완료»만.
**화면**: 메모 글 아래 `data-memo-when` 줄(11px #9a8f76 tabular-nums), 줄 정렬 flex-start·체크 상자 margin-top 1px. 템플릿 3벌(선생님 일지·학원 일지 등) 동일(R4). 휴대폰 390px 정상.

### 8-27. 학원 시험 자동 채점 · 성적통지서 · 반 결과판 (2026-09-15, v33.193, 원장 승인)
**흐름(원장 확정)**: ① 자동 채점 창 1·정답키에 원장이 만든 «정답해설 PDF + 문제지 PDF»를 함께 올림 → 정답·배점·영역·문항 수·시험명·학년 자동 ② 2·학생 답·채점에 학생들이 마킹한 OMR 카드 여러 장(스캔 PDF/이미지) → 생년월일로 식별·채점(기존 경로) ③ 3·알림장이 학원 시험이면 «학생 통지서 | 반 결과판». **모의고사 정답·해설 PDF 경로는 그대로**(원장 «모의고사는 틀이 같으니 그대로»).
**정답키**: «문항 수» 칸 + 20/25/30/45 버튼(`setExamCount`, 1~45, 줄인 문항은 `es.qStash`에 보관했다 늘리면 복원). 배점 소수 허용 — `parseInt(q.pt)` 11곳 → `_ptNum`/`_esTotal`, 점수는 `Math.round(x*10)/10` 누적(3.3점 잘림 버그 수정). `setExamQ`로 정답·배점을 고치면 `_esRegrade`가 이미 채점한 학생·성적일지 기록까지 다시 채점. 새 시험 1~17번 «듣기» 기본값은 모의고사용이라 유지.
**PDF 판별** `importExamKeyPdf` → `_pdfTwoModes`(pdf.js, 같은 줄 ±3pt 묶음 `flat` + 좌우 단 분리 `cols`) → `_acadParse`: 글자에 «[출제의도]»·«정 답»(띄어 쓴 모의고사 표기)이 있으면 **건너뛰고 `_importMockKey`(옛 importExamKeyPdf)**. ⓐ 쪽집게형(sheet): 정답해설의 «N. ③» 해설 줄 + 정답표(번호 줄/동그라미 줄) 대조, «어휘 (1~4번)» 제목 = 영역(q.group), 해설 문장 = q.exp(240자), 문제지 `cols`에서 «N.» 발문 구간의 «(4점)» = 배점(«부 록» 이후 제외), 머리말 «25문항»·«(100점)»으로 검산. ⓑ 표형(table): «번호|출처|원유형→변형|정답|배점» 한 줄 = key·pt(소수)·type(→ 뒤)·src. 어긋나면 q.chk=1(정답키 줄 코랄 테두리) + 안내. 적용 `_applyAcadKey`: `es.kind='academy'`, `es.acad{total,hdrTotal,form}`, 이름은 파일명에서 «_정답해설·_3판260912·-복사» 제거. 모의고사로 다시 채우면 `es.kind=''`.
**검증 실측**: 원장 PDF 5종(항도중1 4강·5강·4-5강 통합심화, 대동고 역설계 1·2차) 정답 125/125·배점 125/125(합 100)·영역/유형 125/125. 발문 단어로 세부 유형 추정은 절반가량 틀려 **넣지 않음**. 스캔 학교 시험지 OCR(동지고 6/25·동성고 0) → 정답·배점 자동엔 부적합(결론 기록).
**학생 통지서** `#schoolcap`(data-capw 680, 이미지 복사·PNG 저장): 딥그린 헤더 «학교시험 대비 성적통지서» → 성명·학교·반·담당 → 총점 도넛·맞은 문항·**응시생 N명 평균**·최고배점 문항 맞힌 수 → 영역별 성취도(강점 딥그린/보완 코랄 막대 + **금색 눈금 = 응시생 평균**) + 레이더(나 딥그린 채움 · 평균 금색 점선) → 답안 지도(✓/정답 번호, 최고배점 문항 금테) → 틀린 문항 짚어보기(내 답→정답 + 정답해설 문장, 6개까지) → A NOTE FOR YOU(`sub.note`, 자동 초안 `_acadNoteFor`, 입력은 `_typSet` 오버레이). **순위는 넣지 않음(원장 결정)**. 렌더값 `acadRep`(`_acadRepVals`/`_acadStats`), 알림장 단계에서만 계산.
**반 결과판**(학부모 전송 없음): 응시·평균·최고·최저·확인 필요 카드(`state.omrLast` = onOmrFiles 결과) → 점수 분포(10구간 + 평균 점선) → 문항별 정답률(영역 색, 60% 미만 코랄 숫자) → 영역 평균 · 정답률 낮은 문항 5(가장 많이 고른 오답 + 해설 첫 문장) → 학생별 결과(누르면 그 학생 통지서).
⚠️ **이 런타임은 SVG `<text>` 안의 `{{ }}` 값을 그리지 않는다**(고정 글자는 보임). 그래프의 숫자·이름표는 HTML(막대는 flex div, 레이더 이름표는 절대위치 div)로 그릴 것. 도넛·레이더 도형처럼 속성 바인딩만 쓰는 SVG는 정상.
검증: `acad_e2e.py` 32항목(PDF→정답키·문항 수 20↔25 복원·모의고사 경로 유지·카드 4장 채점·소수 3.3 재채점·정답 수정 재채점·통지서·한마디 저장·html2canvas·결과판·학생 줄 이동), `acad_parse_test.py` 5종, his-exam 27, omr25_e2e.

### 8-28. 채점 규칙 확정 — 반 구분 없음 · 총점 항상 100 · 문항 수 자동 (2026-09-15, v33.194, 원장 지시)
**지시**: «모의고사나 학교시험 대비 채점은 반에 상관없이 OMR 인식» · «총 배점은 무조건 100점» · «문항 수는 누를 필요 없고 자동 인식». (원장 화면: 선택 반 5명이 «미입력»으로 깔리고 권주은 27/53 · 20/25/30/45 버튼)
**원인**: OMR 식별(`_omrMatch`)은 이미 전 학생 대상이었지만 2단계 목록이 `activeClassId` 반 학생을 먼저 깔고 빈 반이면 «반을 선택하세요» → 반에 묶인 것처럼 보임. 점수는 문항 배점을 그대로 합산해 기본 2점짜리 빈 문항까지 만점에 섞임.
**규칙(채점 계산은 이제 한 곳)**: `_esQs(es)` = 채점 대상 문항 — 학원 PDF 시험은 PDF 문항 전부, 그 외는 **정답이 들어간 마지막 번호까지**(정답 0 이면 45 전부). `_esTotal` = 대상 배점 원합계. `_esScale(es, raw)` = 원합계가 100 이면 그대로, 아니면 `raw/합계×100`(소수 1자리). `_esGrade(es, answers)` → {score(100점 만점), correct, n, wt}. setExamAnswer·applyOmr·_ltRecordSub·_esRegrade·학생 답/알림장 렌더값·통지서 통계가 모두 이것을 씀 → 기록 `rec.total` 은 항상 '100'. 창을 열 때(`openExamGrade`) 현재 시험을 `_esRegrade` 해서 옛 27/53 같은 저장값도 새 규칙으로.
**화면**: 정답키 «문항 수 N 자동»(버튼·입력칸 제거, 범위 밖 줄은 흐리게), «총 배점 100» + 원합계가 다르면 «배점 합 53점 → 100점으로 환산». 2단계 = «채점된 학생 N명 · 반 구분 없음»(채점된 학생만, 반 이름 표기) + «직접 입력할 학생 이름 찾기 (모든 반)» 검색(퇴원생 제외, 12명까지). `setExamCount` 메서드는 남아 있으나 화면에서 부르지 않음.
**v33.208 응시생 수 삭제(원장 «응시생 몇 명인지는 적지 말고 평균만»)**: 학원 시험 통지서(#schoolcap) `acadRep.c.nLabel` = «응시생 전체 평균 점수»(v33.211 원장 확정 — v33.208 «평균» → v33.210 «응시생 평균 점수» → v33.211; 예전 «응시생 N명 평균»).
**v33.211 «단계별로 보기» 버튼 삭제(원장 지시)**: 넓은 화면은 항상 한 화면 3단. `examWideOff`·`onExamWideToggle`·`exWideLabel` 렌더값은 남아 있으나 버튼이 없어 켤 방법 없음(좁은 화면은 여전히 단계 탭). 선생님용 반 결과판 «응시» 칸은 유지(학부모 전송 없음). 성적통지서(#examcap)는 원래 인원 표기 없음.
**v33.209 한 화면 A안(원장 승인)**: 창 폭 1280px 이상이면 정답키 · OMR 채점 · 통지서를 3단(0.85:1:1.25)으로 동시 표시 — `_exWide()` = 창 열림 && `!examWideOff` && innerWidth≥1280. 단계 스위치(isExamKey/isExamGradeStep/isExamReport)와 무거운 계산(examAnswer·acadRep·examNotice·examGradeReport)이 넓은 화면에선 모두 켜짐. 단계 탭은 `exNotWide` 일 때만, 머리에 «단계별로 보기 ↔ 한 화면으로 보기»(`examWideOff`). 칸마다 `data-excol` · 제 높이 스크롤(`calc(100vh - 128px)`).
통지서 맞춤 = `[data-exfit]` 상자 안 `[data-exfit-in]` 을 680px 고정 후 `_exFit()`(componentDidUpdate + 창 크기 변경)가 `transform:scale` — **기본 «크게 보기»**(`examFitMode` 없거나 'w' = 칸 폭에 맞춤, v33.210 원장 지시), «전체 한눈에»(`'all'`, 칸 높이·폭에 맞춰 통째로). ⚠️ 축소는 **바깥 상자에만** — 캡처 대상 #schoolcap/#examcap 자체에는 transform 을 걸지 않아 카톡 이미지 680px 그대로(복제본 캡처). 검증 `wide209_e2e.py` 12항목.
**v33.200 모의고사형 가로 막대 파스텔(원장 «가로막대 색깔 파스텔 톤»)**: 성적통지서 가로 막대(유형 9개 이상)만 별도 값 `hc`/`hlc` — 강점 #86C4A2 · 보통 #C3E2CF · 보완 #F3B3A5, 퍼센트 글자 #2F7A55 · #4E7A63 · #B25B4C, 바탕 #F2EDE2 10px·둥글기 6px. 세로 막대(`color`/`lc`, 레벨테스트형)와 신규생 통지서는 그대로. 검증 `pastel200_e2e.py`(v33.199 18항목 + 색 3항목).
**v33.199 성적 알림장 → 성적통지서(원장 «신규생 영어능력시험 성적통지서처럼, 수업시간표·안내회신 제외, 제목 성적통지서»)**: 학원 PDF 가 아닌 시험(모의고사·직접 입력·레벨테스트 세트)의 3단계를 `#examcap` 새 틀로 교체(렌더값 `examNotice` = `_examNoticeVals`, 통계는 `_acadStats`·점수는 `_esGrade` 공용). 구성 = `#intakecap` 과 같은 뼈대: 로고 헤더(제목 레벨테스트 세트 «영어영역 능력시험 성적통지서», 그 외 «영어 {모의고사|학교시험|학원시험} 성적통지서» — `_examCat` 로 이름 분류) · Class·배정 반(그룹 수업/1:1) + Teacher·담당(`{owner}T`, 관리자=원장) · **시간표·Notice 없음** · 성명/생년월일/성별/출신교/과목 표(intake.birth·gender, school) · 영역별 성취도(유형 3~8개 = 레이더 + 세로 막대 + 평균 점선, 9개 이상 = 가로 막대 2열 + 평균 점선, 강점/보완 칩) · 총점 도넛 + 등급 알약 + 유형별 답안 지도(✓/정답 번호, 고배점 금테, 두 칸 «?») · 진단(자동 문장 + 약한 유형 틀린 번호 형광 + «선생님 한마디» `sub.note`) · **원점수 등급표**(1등급 90~100 … 9등급 0~19, 내 등급 강조 — 레벨테스트용 백분위/100명중 표는 원점수 등급과 뜻이 달라 쓰지 않음 — ⚠️ 레벨테스트 세트의 등급·등급표는 v33.262 부터 판정 반 기준, §8-60). 3단계에서 바로 학생 알약으로 고르기, 이미지 복사·PNG 저장, 단계 이름 «3 · 통지서», 파일명 «HIS_성적통지서_이름». 학원 PDF 시험은 기존 `#schoolcap` 유지. 유형이 9개 이상(모의고사)이면 강점·보완 칩과 진단 문장은 3개까지 «외 N», 답안 지도는 유형별 대신 **10문항씩**(«1~10번 … 41~45번», 한 줄 15칸은 통지서 폭 680px에서 줄바꿈되어 폐기). 검증 `notice199_e2e.py` 18항목(레벨테스트 4영역 · 모의고사 17유형).
**v33.198 시험 목록 이름 바꾸기 · 삭제 · 빈 시험 정리(원장 «삭제나 수정 기능»)**: 목록 줄(`data-exam-set-row`)마다 «이름 바꾸기»(그 자리 입력칸 `data-exam-set-edit`, Enter·저장 / Esc·취소, `renameExamSet` 은 keyT 갱신) · «삭제»(`deleteExamSet`: confirm — 채점 기록이 있으면 «채점된 N명의 성적 성장일지 기록도 함께 지워집니다»). 삭제는 `_examSetDrop` 이 **`dd.deletedExamSets[]` 표식 + 그 시험의 성적 기록 id 를 `deletedExams[]` 에** 넣고 둘 다 제거 → `mergeAppData` 가 `deletedExamSets` 합집합 후 examSets 에서 제거(deletedExams 와 같은 패턴) → 옛 사본이 와도 안 되살아남. 열린 시험을 지우면 `_examSetPickAfterDrop`(가장 최근 다른 시험, 없으면 빈 새 시험). 목록 머리 «빈 시험 N개 정리»(`cleanEmptyExamSets`: 이름 공백 · 채점 0명 · 열린 시험 제외). 레벨테스트 3종(lt2026*)은 버튼 없음 · 호출도 무시(앱이 자동 생성). 공백 이름은 «이름 없는 시험»으로 표시. 검증 `sets198_e2e.py` 15항목.
**v33.197 시험 종류 분류(원장 «중간 쪽집게는 학원시험»)**: 성적 성장일지 추이 분류기 `_examCat` — ① 기록의 `category` 가 3종이면 그대로 ② 기록이 연결된 시험(`examSetId`)이 학원 정답해설 PDF 로 만든 시험(`kind==='academy'`)이면 이름과 상관없이 **학원시험** ③ 이름에 «학원·쪽집게·족집게·대비·예상·적중·변형·역설계·파이널·단원·주간·월말·리마스터·어휘·구문·문법·레벨»이 있으면 학원시험(**«중간·기말»보다 먼저** 봄) ④ «중간·기말·내신·학교·지필·수행» → 학교시험 ⑤ 나머지 모의고사. 이미 저장된 기록도 저장값을 바꾸지 않고 바로 새 분류로 보인다. 검증 `cat197_e2e.py` 9항목(중간 쪽집게·중간고사 대비·기말 역설계 → 학원시험, 2학기 중간고사 → 학교시험, 전국연합 → 모의고사, 추이 학원 4 · 학교 2 · 모의 1).
**v33.196 S급 검수 5건 수정(원장 «1~5순위 전체 순서대로»)** — 검수에서 실제 앱으로 5건 모두 재현(`review_exam.py`) 후 수정:
① **두 칸 마킹**: `_omrReadCanvas` 가 답 칸(가장 진한 칸) 외에 둘째로 진한 칸이 `밝기<205` 이고 `나머지 세 칸 평균 − 둘째 > 30` 이면 `amb[q]=[두 후보]` 를 함께 돌려준다(`ans` 는 그대로 — 레벨테스트 경로 영향 없음). 시험 채점 `onOmrFiles → applyOmr(sid, ans, amb)` 는 그 문항을 **답에서 빼고 `sub.amb` 에 보관 → 확정 전에는 틀린 것으로 셈**. 선생님이 칸을 누르면(`setExamAnswer`) 해제. 화면: 채점 안내 «두 칸 마킹 확인 필요 N명: 이름 3·7번», 학생 알약 코랄 테두리 «확인 N · 점수», 답 줄 «? · ④⑤ 골라요» + 후보 칸 분홍, 안내 줄(`data-amb-note`), 통지서 위 경고(`data-amb-warn`, 캡처 밖), 결과판 «확인 필요 카드»에 합산. **실측 기준**: 실제 학생 카드 스캔(SCAN/20260907182851.pdf, 45문항)에서 정상 44문항의 «나머지 평균 − 둘째» 최대 8, 실제로 두 칸 칠한 42번(③137·⑤128)은 105 → 기준 30 은 양쪽에 여유. 예전 판독기는 이 카드 42번을 조용히 ⑤로 채점했음.
② **배점 없음 → 균등**: `_esGrade` 가 배점 합 0 이면 `맞은 수/문항 수×100`. 정답키 «배점 정보 없음 → 문항마다 4점씩 똑같이», 학원 PDF 안내 «배점 균등 100점».
③ **기기 간 최신 우선**: 시험 세트 `keyT`(setExamQ·setExamMeta·setExamCount·PDF 채움·새 시험에서 갱신), 답안 `sub.t`(applyOmr·setExamAnswer·setAcadNote). `mergeAppData` examSets = `keyT` 큰 쪽 세트 + submissions 는 학생별 `t` 큰 쪽(같으면 over). 옛 데이터(keyT·t 없음)는 예전처럼 over.
④ **시험 목록**: 채점 창 머리에 현재 시험 이름 버튼(`data-exam-sets-btn`) → 목록(`data-exam-sets`, 최근 수정 순, 레벨테스트 3종은 맨 아래, «날짜 · 채점 N명»), 고르면 그 시험 재채점 후 열림. 새로 열 때(`openExamGrade`)는 레벨테스트가 아닌 가장 최근 시험.
⑤ **재채점은 바뀐 점수만**: `_esRegrade` 가 점수·만점·등급·틀린 유형이 실제로 달라진 학생만 고치고(시험명·날짜 등 기록 칸은 안 건드림), 그 학생만 `_ltSyncIntakeFromSub` → 레벨테스트 상담 기록의 손으로 고친 영역 정답 수 보존. 바뀐 수를 돌려줌.
검증: `fix196_e2e.py` 20항목 + 실제 스캔 오탐 검사(`real_amb_check.py`/`real_vals.py`) + `acad_e2e.py` 32 + `fix194_e2e.py` 18.
...
### 8-32. 정답해설 PDF 읽기 — 배치 차이에 너그럽게 (2026-09-21, v33.214, 원장 신고 «유강중1 올렸는데 정답·배점을 찾지 못했어요»)
원장 눈에는 항도중과 **같은 양식**인데 PDF 내부 배치가 달랐다: ① 해설이 «N. | ③ | 설명» **한 줄·2단 표**(항도중은 «N. ③» 단독 줄) ② 정답표 둘째 줄 끝에 «계 / 100점» 칸 ③ 영역 제목(«어휘 (1~4번)») 없음 ④ «포 항 유 강 중 학 교»·«1학년»이 떨어져 있음.
`_acadParse` 수리: ⓐ 기존 해설 읽기로 5개 미만일 때만 **보조 읽기**(토큰 단위 «N.»+동그라미 / «N. ③ 설명», `cols` 먼저·`flat` 다음, 더 많이 읽은 쪽) — 기존 양식 결과 불변 ⓑ 정답표 줄에 숫자·대시 아닌 칸 1개까지 허용(그 칸 아래는 동그라미가 아니어야 함) ⓒ 학년 보조(«중 학 교»+«N학년»).
검증 `yg_parse.py`(앱의 실제 `_pdfTwoModes`+`_acadParse` 를 PDF 에 직접 실행): 유강중 심화·기본 25/25·합 100(정답은 정답표 육안 대조, 배점은 pymupdf 독립 추출과 일치) · 항도중 3종 고치기 전과 출력 완전 동일 · 새 «45문항 4판» 45/45·합 100.
**v33.215 영역 자동 추정(원장 «영역 제목 없어도 스스로 인식·생성·수정»)**: 정답해설에 영역 제목이 없는 문항은 **문제지**에서 추정 — ① 지문 묶음 «※(a~b) 다음 글을/대화를 읽고…»에 속하면 그 묶음의 영역(지문이 «Name:»으로 시작하거나 «대화»면 의사소통, 아니면 본문) ② 그 외는 발문 키워드 순서대로: 어법(어법·문법·영작·영어로 바르게 옮·원형과 과거형·쓰임이 같은·번째로 오는·단어를 배열) → 어휘(품사·관계·영영·풀이·어휘·해석·뜻·의미로 쓰인·공통으로·단어) → 의사소통(대화·의사소통·의도·표현, 또는 «A:» 대화 줄) → «빈칸»=어법 → «윗글/글»=본문. **제목이 있으면 항상 제목 우선**(`res.estN` = 추정한 문항 수, 안내 «영역은 발문으로 추정(N문항) — 유형 칸에서 고칠 수 있어요», 빨간 확인 필요 아님).
정확도: 영역 제목이 붙은 4종 120문항 대조 **119/120**(빗나간 1건 = «대화의 빈칸 (가),(나)»로 위장한 어법 문항 — 발문만으로는 구분 불가) · 유강중 2종 50문항 육안 확인. 보조 읽기도 영역 제목을 모음(40자까지, «본문 — Lesson 4 … (17~27번)»은 «본문»으로) → 45문항 4판은 제목 기준(추정 0).
`setExamQ` 의 `group`/`type` 수정도 `_esRegrade` → 이미 채점된 학생의 틀린 유형·통지서가 따라 바뀜. `openExamGrade`: 일반 시험이 하나도 없으면 레벨테스트 세트 대신 **새 빈 시험**(예전엔 «레벨테스트 2026 · 고등»이 열려 PDF 정답키가 그 위에 쓰일 수 있었음) + `importExamKeyPdf` 가 lt2026 세트엔 거부 안내. 검증 `area215_e2e.py` 11항목(실제 파일 입력으로 유강중 PDF 올리기) · `area_proto.py`(정확도 측정).
...
### 8-63. OMR 판독기 스캔 판독 멈춤 수정 (2026-09-24, `HIS-OMR판독기.html` 만 · 앱 v33.264 그대로)
- 발견(레벨테스트 전수 검수 — 인쇄용 답안지 PDF 에서 동그라미 중심을 뽑아 칠한 합성 스캔으로 판독 시험): 판독기 `scoreOf` 가 `readCard` 의 `ans`(객체 {문항:답})에 배열 전용 `.filter` 를 불러 TypeError → `readAuto`/`processToResult` 가 매번 멈춤. **v32.980(07-28, 4방향 자동 회전)부터 판독기에 스캔을 올려도 채점되지 않았음.** 앱 `_omrReadAuto` 는 `Object.keys` 로 세어 문제 없음(신규생·시험 도구 OMR 정상).
- 수정: 배열·객체 모두 셈. 검증 `omr_read.py`(반듯 · 거꾸로 · 1.2° 기울고 잡음 3장 — 판독기·앱 모두 45문항·생년월일 일치, 애매 0) · `omr_ui.py`(파일 칸으로 3장 올리기 → 배치 3줄 · 총점 = 정답키 채점 · 통지표 «Form E (초등 5–6)»).

### 8-62. 레벨테스트 «평균 N등급»도 판정 반 · 영역 막대 등급 글자 빼기 (2026-09-24, v33.264, 원장 «1 a · 2 a»)
- **2a** — `ltSummary(lt, data, sid)`: 학생 id 를 주면 OMR 답(`_ltG9For`)이 있을 때 `avgGrade` = 판정 반 등급 + `lvGrade:true`, 없으면 예전 `ltGrade(평균)`. 부르는 곳 6곳에 id 전달 → 종합일지 `#profilecap`(«평균 92% · 6등급»·진단 대체 문장) · 종합일지 상담 요약 `csLt` · 상담 참고 패널(`exLt`, 내부용) · 홈 «곧 함께하는 학생» 카드(`_lt`) · 엑셀 2종(`exportStudentCSV`·`exportAllStudentsCSV` — OMR 학생은 칸 이름도 «등급(판정 반)», 수기 학생은 «평균 등급» 그대로). 신규생 창 `liveLt`(초안)는 id 없이 부르므로 그대로(통지서 등급은 §8-61 `ltGradeShowNum`).
- **1a** — 신규생 통지서 영역 막대 아래 «N등급»(`{{ cb.grade }}`) 삭제 → 영역은 %만(영역에는 반이 없음). 같은 이유로 상담 참고 패널 막대 옆 «N등급»(`{{ b.grade }}` 38px 칸)도 삭제. rv 의 `grade` 값은 남아 있으나 화면에 안 씀.
- 수기 입력 학생(OMR 답 없음)은 어디서나 예전 그대로.
- 검증 `lt264_e2e.py` 11항목(요약 함수 · 종합일지 OMR/수기 · 상담 패널 · 홈 카드 OMR/수기 · 엑셀 3 · 신규생 통지서 등급 글자 10개 · 콘솔 0) — 고치기 전 실행에서 6항목 실패(옛 값 확인) 후 전부 통과 · `lt262_e2e.py` 29/29 · `sweep_0924.py` 9,009장 · his-check 새 위반 0(기존 2건) · his-exam 27/27.

### 8-61. 신규생 통지서 등급도 판정 반 기준 (2026-09-24, v33.263, 원장 «3 a» 마저 적용)
- v33.262 가 `#examcap`(시험 도구의 레벨테스트 세트)만 바꾸고 **신규생 통지서 `#intakecap`**(제목이 같은 «영어영역 능력시험 성적통지서», 원장이 실제로 보내는 통지서)을 빠뜨려, 같은 학생이 앱 안에서 6등급/1등급으로 갈렸던 것을 마저 고침.
- `_ltG9For(data, sid, subj)` = lt2026 세트 제출 답(10문항 이상 — `_ltAnswerMap` 과 같은 조건) → `_ltJudge` → 판정 반 등급. `ltGradeShowNum` = OMR 채점 학생(`ltMapA`)이면 이 값(못 구하면 예전 `ltGrade(총점)`), **수기 입력 학생은 예전 그대로** `ltGrade(평균)`(반을 계산할 답이 없음).
- 등급표 강조 `_myG` = `ltGradeShowNum`(예전엔 영역 평균으로 따로 계산해 알약과 한 칸 어긋날 수 있었음 — §1-1 ⑤ «표 등급 칸도 동일 기준» 의도대로), 자동 해석 문장 `ltInterpret`·«자동 문장»(`ltFillDiagnosis`)도 같은 등급. 초등 92점 OMR → 알약 6등급 · 표 «6등급 60~77%» · 중위권 문장.
- §1-1 ⑤(2026-09-01 «도넛·표 등급 = 배점 기준 실제 점수»)의 **등급 기준은 이것으로 바뀜** — 점수(도넛)는 그대로, 등급만 판정 반.
- 그대로 둔 것(원장 확인 대기): 영역 막대 아래 «N등급»(영역 정답률 `ltGrade(s.pct)`) · 종합일지 `#profilecap`·상담 참고 패널·엑셀 내보내기의 «레벨테스트 평균 N등급»(`ltSummary().avgGrade`).
- 검증 `lt262_e2e.py` 29항목(+신규생 통지서 OMR 초·중·고 3 + 수기 1) · 교차 판정 `sweep_0924.py` 9,009장 · his-check 새 위반 0(기존 2건) · his-exam 27/27.

### 8-60. 레벨테스트 등급 = 판정 반 기준 · 신규생 OMR 재응시 안내 · 판독기 재응시·시험지 이름 복원 (2026-09-24, v33.262, 원장 «1 고쳐 · 3 a»)
- 발견(레벨테스트 전수 검수): 같은 학생의 등급이 통지서마다 달랐다 — 앱 `#examcap` = 원점수 등급(`_gradeOf`, 초등 92점 → 1등급), OMR 판독기 통지표·report.py 앱양식 통지표 = 판정 반에서 옮긴 등급(원장 지시, 초등 92점 → GI 이상 → 6등급). 원장 «a» = **판정 반 기준으로 통일**.
- `_ltGrade9(level)`(판독기 `LG`·report.py `LEVEL_GRADE` 와 같은 표, «이하» 구간은 한 등급 아래) · `_ltJudge(subj, answers)`(`_ltApplyOmr`/`_ltSyncIntakeFromSub` 와 같은 판정 규칙 + 재응시) · `_esGradeNum(es, answers, pct)` = lt2026 세트면 판정 반 등급, 아니면 `_gradeOf(pct)` 그대로. `_gradeOf` 직접 호출 8곳(setExamAnswer·applyOmr·_ltRecordSub·setExamSubj·_esRegrade·_examNoticeVals·채점 목록 2곳) → `_esGradeNum`. 모의고사·학교시험 등급은 바뀌지 않음(his-exam 27/27).
- 통지서 등급표: 레벨테스트 세트는 둘째 줄 이름 «레벨» + 9칸 = 반 사다리(1 H-AD·H-H / 2 H-M / 3 H-L / 4 M-AD / 5 M-H / 6 M-M·GI / 7 M-L·GB / 8 E-A·JA / 9 그 아래), 그 밖은 «원점수» 점수 구간 그대로(`examNotice.c.gradeRow`).
- `_esRegrade`: 점수·백분율·틀린 유형이 같고 **등급만** 바뀐 학생은 `_ltSyncIntakeFromSub` 를 부르지 않음 → 새 등급 기준으로 처음 재채점할 때 손으로 고친 상담 기록 영역 정답 수가 덮이지 않음(§8-28 ⑤ «재채점은 바뀐 점수만» 유지). 이미 저장된 옛 등급은 재채점·다시 채점할 때 새 기준으로 바뀐다(데이터 일괄 수정은 하지 않음).
- `_ltOmrKeys` 에 `retake`·`label`(판독기 KEYS·레벨테스트 `src/_keys.js` 와 같은 구조). 신규생 OMR 채점(`_ltApplyOmr`) **상태 문구** 끝에 측정 범위 밖이면 재응시 안내(초등 75↑ → Form M · 중등 25↓ → Form E, 85↑ → Form H · 고등 30↓ → Form M — 정답해설 CUT_NOTE·report.py RETAKE 와 같음). 학부모에게 가는 진단 문장에는 넣지 않음.
- `HIS-OMR판독기.html`: KEYS 에 `retake`·`label` 복원 — v32.979(07-28) 정답키 재동기화 때 빠져 통지표에 «Form E (undefined)»가 찍히고 재응시 안내가 사라졌던 것. ⚠️ **정답키를 다시 맞출 때 retake·label 을 빼지 말 것.**
- 검증 `lt262_e2e.py` 25항목(버전 · 3세트 판정·저장 등급·통지서·등급표 · 모의고사 회귀 2 · 신규생 재응시 7경우 · 재채점 보존 2경우 · 콘솔 오류 0 — 커밋 메시지의 «26»은 잘못 센 것) · his-check 새 위반 0(기존 2건 그대로) · his-exam 27/27.
- ⚠️ 여기서 **신규생 통지서 `#intakecap` 이 빠졌음** → §8-61(v33.263)에서 마저 적용.

### 8-59. 모의고사·레벨테스트 성적통지서 머리글 로고 36px (2026-09-24, v33.261, 원장 «모의고사 성적통지서 로고크기도 줄여»)
- `#examcap`(examNotice) 머리글 로고 45 → 36px(`data-notice-logo`, `flex:none`). 이 머리글은 모의고사(«영어 모의고사 성적통지서»)와 레벨테스트 시험 세트(«영어영역 능력시험 성적통지서»)가 함께 쓴다 → 세 통지서(§8-53 성취도평가 · §8-54 신규생 · 이것) 로고가 모두 36px. 앱 안에 45px 크림 로고 머리글은 이제 없다(성장일지·상담 등 크림 바탕 머리글의 짙은 로고 45px 는 별개, 그대로).
- 검증 `logo261_e2e.py` 3항목: 레벨테스트(lt2026M)·모의고사(MK) 통지서 각각 로고 36px · 간격 12px · 좌 여백 20px · 세로 중심 차 0 · 제목 한 줄.

### 8-58. 급여명세서 도장 두 줄 = 진짜 테두리 (2026-09-24, v33.260, 원장 «도장도 고쳐»)
- 도장 span(`data-seal`)에 `position:relative`, 안쪽 선은 `data-seal-line` = `position:absolute; inset:2px; border:1px solid #C1272D; border-radius:2px`(글자 2×2 격자에는 안 끼어듦 — 격자 컨테이너의 절대 위치 자식). 겉 테두리 2.5px·크림 틈 2px·색·크기 52×52·글꼴 그대로. 안쪽 선은 1.5px(그림자)였으나 배율 1 화면에서 어차피 1px 로 깎이므로 처음부터 정수 1px.
- 이제 **템플릿 안에 `box-shadow:inset` 이 하나도 없다**(전수 재확인). JS 값에 남은 안쪽 그림자(필터 타일 강조 `allRing` 등·일정 칩 `sp.sh`)는 캡처 카드 밖 화면 요소.
- 검증 `seal260_e2e.py` 4항목: 화면(그림자 없음·안쪽 선 1px·2×2·52×52) + 앱과 같은 경로의 캡처 PNG 를 픽셀로 훑어 네 변 모두 «겉 테두리 + 안쪽 선» 빨간 줄 2개(`seal260_zoom.png`).

### 8-57. «번호 | 정답 | 해설» 정답표 · 따로 번호 매긴 서답형 · 묶음 배점 읽기 (2026-09-23, v33.259, 원장 «영신고1 변형문제 1회 시험지·정답해설 PDF를 자동 채우기가 못 읽어»)
- **원인**: 정답해설이 표(한 줄 = «1 | ① | 해설…»)인데 판독기 네 방식이 모두 안 맞았다 — 표 방식은 «번호|정답|**배점**», 정답지 방식은 «1. ①»(점 필요), 범용 구역은 «1.»·«1)»·«1번» 표시 필요, 범용 짝은 한 줄에 3쌍 이상. 끝내 `_importMockKey` 가 «스캔한 그림 PDF일 수 있어요»라는 **틀린 안내**를 냈다.
- **수리(`_acadUniParse` 에 `rowsMode`)**: 줄 머리 «번호 + ①~⑤»(한 줄 한 쌍, `glued` 꼴 포함)를 문항 시작으로, 다음 줄들은 해설로 이어 붙임. 번호가 1부터 90% 이상 이어지고 문서 순서대로 늘어날 때만 인정, 한 줄에 쌍이 여럿(정답 격자)이면 빠짐(짝 방식 몫). 머리글 «문항 정답 해설»·«정답 및 해설»·꼬리말(«히즈어학원» 추가)에서 이어 붙이기 멈춤. 같은 문서의 평면 줄(flat)이 두 단 분리(cols)보다 해설이 온전해 동점이면 평면(+3). 복수정답(«①,③»)은 multi 서술형.
- **서답형**: 정답표의 «서답형 N» 줄 → «모범답 — 해설»(첫 «—»에서 나눔). 선택형 L개 뒤 **L+N 번 서술형(sub)** 으로 붙임(영신: 서답형 1~4 = 29~32번, 결과 문구 «서답형 1~4는 29~32번(서술형)으로 넣었어요» = `res.note`). 배점·영역은 시험지 «[서답형 N] (X점) 발문». 선생님이 2 · 학생 답·채점에서 O·△·✕.
- **시험지 배점 보완**: «[a~b] … (각 X점)» 묶음 배점(문항 자기 배점이 없을 때), 묶음 발문으로 영역 추정(문항 발문에 단서가 없을 때). 이름에서 «_시험지» 뗌.
- ⚠️ **pdf.js 가 한글을 글자마다 띄어 뽑는 PDF**(영신 시험지: «다 음 빈 칸 에», «[ 서 답 형 1] (3.6 점 )»): 숫자 규칙은 원래 띄어쓰기 허용이라 읽혔지만 한글 단서가 안 맞았다. → 서답형 머리글은 «서[ ]*답[ ]*형» 허용, **영역 추정은 2단계**(`AR9`): ① 예전 규칙 그대로(+새 단서: 어법 «우리말과 같은 뜻·어구를 모두 사용», 본문 «가리키는·본문에서 찾아»(어휘 «단어»보다 먼저), 어휘 «철자·문맥상») → ② 아무것도 못 찾을 때만 띄어쓰기를 모두 뺀 글·단서(`AR9S`, 단서 정규식 source 에서 공백 제거)로. 처음엔 띄어쓰기 무시를 1단계에 넣었다가 항도중2 5번 «대화가 되도록 … 배\n열하시오»(의사소통)가 «배열하»(어법)로 바뀌는 회귀를 비교 검사로 잡아 2단계로 바꿈 — 항도중2 30문항 영역이 변경 전과 완전히 같음 확인(`area_diff.py`).
- 검증 `ys259_e2e.py` 19항목(1·2·3회 정답 84개를 원본 `_정답해설_원본.html` 정답표와 한 글자씩 대조 · 선택형 85+서답형 15=100 · 서답형 모범답 · 영역 32개 · 해설 32개 · 16번 «⑤ 형용사…» 해설에도 정답 ⑤ · 채점 85→100→94.4(△=0.5) · 정답해설만 올려도 32문항). 판독기 회귀 13종 통과. 시험 PDF 3쌍은 `_tools/hisbooks-test-pdf/영신1/` 에 복사(로컬 전용, 커밋 안 함).
- ⚠️ `list220_e2e.py`(항도중2 목록형, 체인 밖)는 v33.221(서술형도 총점에 합침) 이전 기대값이라 **변경 전(v33.258)에도 똑같이 5건 실패** — 이번 변경과 무관, 기대값 갱신은 따로.

### 8-56. 모든 «이미지 복사» 버튼 = 만드는 중 → ✓ 복사됨 → 원래대로 (2026-09-23, v33.258, 원장 «신규생 진단 이미지 복사도 일간일지처럼 버튼색 변하며 복사됨» → «앱 안의 다른 이미지 복사 버튼도 똑같이»)
- **공통 장치**: `_cpMark(key, st)` 가 `state.cpFx[key]` 에 `'busy'`/`'done'`/없음 을 둠(`done` 은 2.6초 뒤 자동 해제, `busy` 는 20초 감시로 풀림). `_cpVals(key)` → `{ v0, v1, v2, css }`. 버튼은 원래 스타일 끝에 `{{ X.css }}` 를 붙이고, 글자 세 벌(평소 · «⏳ (이미지) 만드는 중…» · «✓ 복사됨(— 카톡에 Ctrl+V)»)을 `display:inline-grid` 한 칸(`grid-area:1/1`)에 겹쳐 `visibility` 만 바꾼다 → **버튼 너비가 절대 들썩이지 않음**(10종 모두 세 상태 너비 동일 실측). 초록 = 일간일지 미리보기와 같은 #1F8A5B · 흰 글자 · 그림자 없음, 만드는 중 = 흐리게(.78) + 진행 커서.
- **초록은 클립보드 쓰기가 실제로 성공한 순간에만**(예전 일간 미리보기는 누르자마자 초록 → 실패해도 «완료»로 보였음). 실패·미지원이면 곧바로 원래대로 + 상태줄에 이유.
- **열쇠 = 무엇을 누구의**: `intake|학생` · `welcome|학생` · `profile|학생` · `school|시험|학생` · `notice|시험|학생` · `pay|이름|월` · `mileage` · `daily|학생|날짜` · `monthly|학생|월` · `cap|종류|학생`. 학생을 바꾸면 초록이 따라가지 않는다.
- **복사 경로 7개 손봄**: `captureEl(id, mode, filename, cpKey)`(4번째 인자 = 열쇠, 없으면 `cap|id` — 다른 호출 18곳은 그대로) · `copyRow(…, onDone)`(일간, `onDone('ok'|'fallback')`) · `copyMileageGuide` · 월간 목록 줄 `ms.onCopy` · 캡처 미리보기 `onCapPreviewCopy`. 글자 복사(문자·안내문·미납 명단·전체 적용)와 `shareEl`(공유)·`copyEl`(종합일지 공유)은 대상 아님.
- **연속 보내기 흐름 변경**: 일간 미리보기(`sendQueue`)와 캡처 미리보기(`capPreviewQueue`)는 예전엔 누르고 0.6초/캔버스 직후 다음 학생으로 넘어가 복사 성공을 확인할 틈이 없었다 → **복사 성공 후 0.7초** 초록을 보여 주고 넘어감(그사이 사용자가 직접 넘겼으면 안 넘김). 일간은 실패하면 넘어가지 않아 다시 누를 수 있음. 휴대폰 대체 경로(이미지 창 띄우기)는 예전 시간(0.6초/0.45초) 그대로. `previewCopied`/`capPreviewCopied` 깜빡임은 폐기(값은 남아 있으나 버튼이 안 씀).
- 버튼 10종: 신규생 진단(`data-cp-btn=intake`, 디자인 시스템 부품 → 같은 클래스 `his-btn his-btn--accent his-btn--sm` 일반 버튼) · 환영 카드 2곳 · 학생 프로필 · 히즈 성취도평가 통지서 · 모의고사·레벨테스트 성적통지서 · 급여명세서 «카톡 복사» · 마일리지 안내 · 일간 미리보기 · 월간 목록 줄 · 캡처 미리보기.
- 검증 `cp258_e2e.py` 17항목: 클립보드를 «실제 이미지 Blob 이 만들어질 때까지 기다렸다 성공/실패» 하는 대역으로 바꿔 캡처 전 과정을 돌림 — 10종 각각 누르자마자 만드는 중(흐림) → 성공 뒤 초록(실제 PNG 9만~190만 바이트) → 2.6초 뒤 원래대로 · 너비 동일, 실패 경로, 학생 바꾸기 격리, 일간 연속 보내기(성공 뒤 넘어감 / 실패면 멈춤), 캡처 미리보기 연속. ⚠️ 테스트에서 배운 것: 신규생 패널은 `closeIntake()` 로 닫는다(`intakeDraft:null` 로 지우면 렌더 전체가 멈춘다) · 색 전환(0.2초) 버튼은 바뀐 직후가 아니라 0.35초 뒤에 색을 잰다.

### 8-55. 3점 변별·고배점 문항 = 금색 2px 진짜 테두리 (2026-09-23, v33.257, 원장 «C안 금색 테두리»)
- 안쪽 그림자(§8-54 발견)를 모두 걷어냄: 칸 템플릿 2곳(`#intakecap` 지도 `data-lt-cell` · 시험 통지서 `data-notice-cell`)을 `box-sizing:border-box;border:1px solid {{ mc.bd }};border-width:{{ mc.bw }};` 로 — 겉 크기 23×25 그대로, `bw` 가 빠져도 1px 로 남는 방어형. 칸 값 3곳: 신규생 OMR 지도(`_ltAnswerMap` p3 → `bd` 금색 #C9A227 · `bw:'2px'`) · 신규생 수기 지도(`_ltManualMap` → `bw:'1px'`, 같은 템플릿이라 필수) · 시험 통지서(`gold` 고배점 → 금색 2px, 두 칸 마킹 `am` 은 종전대로 허니 바탕 + 금색 1px). 범례 칩 2곳(`data-p3-legend`) 10×10 흰 바탕 + 금색 2px 테두리.
- **두께 2px 인 이유**: 크롬은 테두리 두께를 화면 픽셀로 깎는다 — 배율 1 화면에서 1.5px 는 1px 로 계산돼(html2canvas 도 그 값을 읽음) 기기마다 굵기가 달라진다. 원장에게 보여 준 시안은 배율 2로 찍어 1.5px 로 보였던 것. 정수 2px 는 배율 1·1.25·1.5·2 어디서나 거의 같은 굵기.
- 틀린 3점 칸은 분홍 바탕 + 금색 테두리(코랄 테두리 대신) — 시안 그대로. 모의고사 성적통지서의 «N점 고배점 문항»도 같은 장치·같은 캡처 깨짐이라 함께 고침.
- 검증 `c257_e2e.py` 8항목: 시험 통지서 45칸 중 고배점 10칸만 금색 2px · 신규생 통지서 3점 10칸만 금색 2px(틀린 칸 포함) · 수기 지도 45칸 1px · 모든 칸 23×25 · 그림자 없음 · 범례 칩 · 두 카드 html2canvas 캡처(`c257_intake.png`·`c257_notice.png`)를 앱과 같은 경로(clone + `_capFit`)로 찍어 눈으로 확인. ⚠️ 수기 지도 테스트 데이터는 `stu.levelTest` 에 `listenC/listenT…` 를 넣어야 뜬다(`openIntake` 가 `levelTest` 를 `intakeDraft.lt` 로 합침, `ltSummary` 는 T 가 있어야 비율 계산).
...
- ✅ **(해결 v33.260, §8-58)** 급여명세서(`#paycap`) 원장 도장 «宋慶根印» 두 줄 테두리 = `border:2.5px` + `box-shadow:inset 0 0 0 2px #F3ECD7, inset 0 0 0 3.5px #C1272D` 였다 — 캡처하면 안쪽 선이 사라지고 모서리 조각만 남았다(`seal_probe.py`).
...
- ⚠️ 통지서 미리보기는 축소 표시(`#schoolcap` 폭 / 680 ≈ 0.76)라 **치수 검증은 그 비율로 나눠서** 본다(`logo255_e2e.py` 5항목).

### 8-52. 학원 시험 통지서 이름 «히즈 성취도평가 통지서» · 머리글 좌우 대칭 (2026-09-22, v33.254, 원장 작명·«시험명 조금 더 크게 대칭 정렬»)
- 제목: «학교시험 대비 성적통지서» → **«히즈 성취도평가 통지서»**(학원 시험 `#schoolcap` 만; 모의고사 «영어 N월 성적통지서»·레벨테스트 «영어영역 능력시험 성적통지서»·저장 파일명 `HIS_성적통지서_` 은 그대로). 작명 근거: 주체가 히즈, «성취도평가»는 학부모가 아는 말, 형제 문서와 같은 구조. 원장이 «성적통지서» 대신 «통지서»로 가볍게 확정.
- 머리글(`data-school-head`): 오른쪽 시험명 블록이 `position:absolute` + 11.5px 라 왼쪽과 짝이 안 맞던 것을 **flex 좌우 두 블록**으로 — 왼쪽(제목 18px/1.3 800 크림 + 구호 이탤릭 11px 금색) ↔ 오른쪽(`data-school-exam`, 시험명 **15px/1.3 800 크림**, `padding-top:2px` 로 윗줄 맞춤 + 날짜 이탤릭 11px 금색 `margin-top:6px`). 오른쪽은 `max-width:62%; min-width:0` 이라 긴 이름은 두 줄로 접힘. 검증 `title254_e2e.py`: 윗줄 차 0px · 아랫줄 차 1.4px · 좌우 여백 0 · 22자 이름 한 줄.

### 8-51. 시험 목록 준비 칩 배치 — 이름 오른쪽 한 줄 (2026-09-22, v33.253, 원장 스크린샷 «이름이 20…으로 잘림» + «아래로 내리지 말고 오른쪽으로»)
- **원인**: v33.249 칩 묶음(`data-exam-ready`, `flex-basis:100%`)을 `display:flex` 이지만 `flex-wrap` 이 없는 행(`data-exam-set-row`)의 **버튼 밖 형제**로 넣어, 줄바꿈 대신 이름 버튼 옆에 끼어 이름을 «20…»으로 눌렀다(테스트는 칩 글자만 확인해 치수를 못 잡음).
- **수리**: 칩 묶음을 이름 버튼 **안**으로 옮겨 «이름(`flex:1 1 auto; min-width:9em`, 말줄임) → 칩(`margin-left:auto`, 오른쪽 정렬, `flex:0 1 auto`) → 날짜·채점(`flex:none; margin-left:auto`)» 한 줄. 버튼은 `flex-wrap:wrap` — 폭이 정말 없을 때(휴대폰 390px, 활성 행은 «이름 바꾸기·삭제» 때문에 128px)만 칩·날짜가 이름 **아래로** 접히고, PC·태블릿(1024px~)에서는 항상 한 줄. 처음 시도한 «항상 아래 줄» 안은 원장이 거부해 되돌림(백업 복원 후 재패치).
- ⚠️ 채점 창 자체가 390px 에서 280px 로 눌리고 머리글 제목이 세로로 서는 것은 이전부터의 상태(이번 범위 밖, 채점은 PC 작업).
- 검증 `chip253_e2e.py` 5항목 — 1900/1024/390 세 폭에서 순서·이름 폭·칩 폭(0 겹침 금지)·칩 없는 행(레벨테스트) 배치 유지 + 스크린샷. 앞으로 **배치 변경은 글자 확인만이 아니라 `getBoundingClientRect` 치수로 검증**한다.

### 8-50. 시험 이름 변경 → 오답노트 카드 · 원장 홈 «통지서 미완성 시험» (2026-09-22, v33.251~252, 원장 승인 T4·T5)
- **v33.251 T4**: `renameExamSet` 뒤 `_clipRenameExam(esId, nm, sids)` — 그 시험에 답안이 있는 학생마다 오답노트(`his_clip_<sid>` 로컬/클라우드 중 최신)를 열어 **자동 카드(`ex_<시험id>_`)만** `exam` 을 새 이름으로 바꾸고 저장(손으로 담은 카드는 그대로). 서랍이 열려 있으면 화면도 갱신.
- **v33.252 T5**: 홈 «통지서 미완성 시험 N개» 카드(`data-home-examtodo`, **관리자만**, «우리 반 챙길 학생» 위) — 최근 45일 학원 시험 중 채점 학생이 있고 «유형 N개 · 두 칸 마킹 n명 · 서술형 미입력 n명 · 해설·메모 없음 n명» 중 하나라도 있으면 표시(최대 6개, 최근순). 누르면 `activeExamSetId` 를 두고 `openExamGrade()`(기존 세트를 존중) → 그 시험 채점 창. 배점 균등은 경고가 아니라 빼둠. 다 채우면 카드 사라짐.
- 검증 `t45_e2e.py` 6항목. ⚠️ 홈 카드는 Playwright 실제 클릭이 하니스 `data-dc-tpl` 겹침 레이어에 막힌다(§2 알려진 한계) → JS click 으로 검증.

### 8-49. 받을 때 시험 세트 합치기 · 시험 목록 준비 상태 칩 · 전원 동작 경고 (2026-09-22, v33.248~250, 원장 승인 T1·T2·T3)
- **v33.248 T1**: `_absorbFresh` 는 `{ data: fresh }` 로 끝나 특별 처리 없는 키는 클라우드 사본으로 **통째 교체**된다. `examSets` 가 그랬다 → 저장 뒤 2초 push 창 안에 폴링이 오면 방금 적은 서술형·메모가 사라질 수 있었음(기존 구조, 입력이 늘어 위험 커짐). 수리: absorb 첫머리에서 `window.__hisMergeAppData({examSets:클라우드, deletedExamSets}, {examSets:내것, deletedExamSets})` 로 **게이트와 같은 규칙**(세트 keyT · 답안 t · subj/qnote 보존 · 삭제 표식)으로 합친 결과만 `fresh.examSets` 에 넣음. ⚠️ absorb 에 아직 통째 교체인 큰 키: `records`(editT LWW 는 merge 에만) · `closedDays` · `sessions` — 같은 창 문제가 있으나 원장 요청 범위 밖.
- **v33.249 T2**: 시험 목록 줄 아래 준비 상태 칩(`data-ready-chip`, 학원 시험만): 유형 ✓ / N개 비움(코랄) · 배점 ✓ / N개 비움(코랄) / **균등**(허니 — 배점 없는 시험지도 있어 경고가 아님) · 해설 ✓ / 해설 k/N(허니) / 해설·메모 없는 오답 N(코랄, 채점된 학생 기준) / 메모 ✓ · 서술형 입력 k/N명. 계산은 `_qualityGaps`·`_subjMiss` 재사용, 목록이 열릴 때만(`_examSetListVals`).
- **v33.250 T3**: `_bulkGuard(es, sids, what)` — «전원 오답 시험지 인쇄»·«전원 오답노트 담기»(결과판 · 모의고사 통지서) 앞에 confirm: 채점 N명 · 서술형 미입력 학생 n명(문항 m) · 두 칸 마킹 · 배점 · 유형 · 해설·메모 없는 오답 학생 n명. 다 채우면 묻지 않음.
- 검증 `t123_e2e.py` 9항목(진짜 클릭 + 대화창 · 폴링 흡수 시뮬레이션 · 삭제 표식). ⚠️ 테스트에서 배운 것: 객관식 배점을 하나라도 손으로 적은 뒤 서술형 배점을 적용하면 나머지는 자동 분배되지 않는다(설계대로) — 그때 경고가 «19개 비움»을 잡는 것이 정상.

### 8-48. 통지서 품질 경고 — 유형·해설·배점 (2026-09-22, v33.247, 원장 «선생님들이 대충하는 것 같아 경고 창» + «배점 표시도»)
- `_qualityGaps(es, sub)`(학원 시험만) → `noType`(유형 빈 문항) · `noPt`(배점 0 문항) · `noExp`(그 학생의 틀린 문항 중 PDF 해설도 선생님 메모(`qnote`)도 없는 것). 세 곳에서 씀:
  ① **정답키 화면 상시 줄**(`data-type-gap`, 코랄 상자) — «유형(영역) 비어 있음 N문항 — 번호» / «배점 비어 있음 N문항 (전부 — 같은 점수로 나눠 계산 중)».
  ② **통지서 위 경고 상자**(기존 `ambWarn` 에 합류, 캡처 밖).
  ③ **보내기 전 `_sendGuard` confirm** — 배점 → 유형 → 해설/메모 순으로 «무엇이 몇 개 · 왜 문제 · 어디서 채우나»를 적고 «이대로 통지서를 만들까요?». 막지는 않고(확인 누르면 진행) 매번 다시 묻는다. 다 채우면 묻지 않음.
- 기준: 배점은 «실제 배점이 다르면 채우라»는 안내(균등 배점이 맞는 시험도 있음), 유형은 «기타»로 나간다는 결과를 명시, 해설은 «학생이 왜 틀렸는지 알 수 없다»로. 검증 `guard247_e2e.py` 8항목(항도중2 = 세 가지 다 비어 있음 → 채워 가며 경고가 줄어드는 순서 · 유강중 = 경고 없음).

### 8-47. 통지서 «틀린 문항 짚어보기» — 해설이 없을 때 (2026-09-22, v33.246, 원장 «해설이 없는 경우 어떻게 할지»)
- 정답 목록만 있는 시험지(항도중2 «1) ④» 양식 등)는 `q.exp` 가 비어 «내 답 ⑤ → 정답 ④» 한 줄뿐이었다. 해설을 지어내지 않고 **채점 데이터에서 나오는 사실**로 채운다.
- 해설이 없는 객관식 문항(응시 2명 이상)마다: ① **반 응답 분포** 5칸 막대(①~⑤ 각 %, 정답 세이지 · 내 답 코랄 · 나머지 회색, `_acadStats().items[].cnt` 에서 계산 — 정답자 수 = rate×n) ② **꼬리표**: 정답률 ≤40% «많이 틀린 문항»(코랄) / ≥80% «꼭 다시 볼 문항»(허니) / 내 오답을 반의 30%+(2명 이상)가 고름 «같은 답을 고른 친구가 많아요» ③ **영역별 학습 안내 한 줄**(`HINT` — 어법·어휘·의사소통·본문·듣기, 메모가 없을 때만) ④ **선생님 메모**(`sub.qnote[문항]`, 120자, 학생별) — 통지서 위(캡처 밖) 크림 상자 «해설이 없는 틀린 문항 N개» 입력 칸(`data-qnote-input`, `_typSet` 오버레이 → `setAcadQnote`), 적으면 통지서에 딥그린 굵은 글로 들어가고 학습 안내는 빠짐. 서술형은 분포 없음. 해설이 있는 시험은 종전 그대로(분포·메모 칸 없음).
- `mergeAppData` submissions: `subj` 와 같은 방식으로 **`qnote` 도 이긴 쪽에 없으면 진 쪽에서 가져옴**(§8-37 v33.223 B 교훈). 검증 `wr246_e2e.py` 7항목(실제 PDF · 4명 채점 · 진짜 타이핑 · 양방향 합치기 · 해설 있는 시험 회귀).
⚠️ 통지서에 **AI 생성 해설을 넣지 않는다** — 학부모에게 나가는 문서라 검증 없는 문장은 금지. 원문 해설(PDF) > 선생님 메모 > 반 분포·학습 안내 순.

### 8-46. 다음 수업 안내에 «그 반만 휴강 / 옮김» (2026-09-22, v33.245, 원장 질문 «선생님 개인 사정·학원 자체 휴원도 같이 나가나» → 승인)
- 실측으로 확인한 상태: 학원 휴원일(`closedDays`)은 날짜+안내가 나가지만, **그 반만 «이번만 휴강»/«다른 날로 보강»(`sessions`)은 날짜만 맞고 안내가 없었다**(학부모는 목요일이 왜 빠졌는지 모름).
- `_wkReport` 가 다음 수업을 찾으며 지나친 날 중 `_sessOv(...).off` 인 날을 `off9` 에 모아 안내: to 없음 → «10.8(목) 수업은 휴강이에요» / to 있음 → «10.8(목) 수업은 10.10(토)로 옮겼어요»(최대 2건). 휴원 안내와 함께면 « · » 로 이음. **휴강 사유(`reason` — «선생님 개인 사정» 등)는 학부모 문구에 넣지 않는다**(원장 제안 승인 사항). 선생님 개인 사정은 학원 전체 휴원일이 아니라 그 반의 «수업 보강»으로 처리하는 것이 맞다(휴원일로 넣으면 다른 선생님 반까지 쉼) — 다른 반 학생 리포트에 영향 없음을 테스트로 고정.
- 말 겹침: 휴원일 이름이 «휴원»/«휴무»로 끝나면 «…로 휴원이에요»를 붙이지 않고 «10.8(목)은 학원 자체 휴원이에요» / «…정기 휴무예요».
- 안내 줄은 `white-space:normal; max-width:340px; text-align:right` — 두 안내가 이어져도 카드 밖으로 안 넘침(실측 340×32px).
- 검증 `off245_e2e.py` 7항목. ⚠️ 큰 테스트 본문도 heredoc 에 넣으면 bash 가 깨진다 → Write 도구로 파일을 만들어 읽어 붙일 것.

### 8-45. 월간 아래 패널 정리 · 보강 예약의 휴원일 · 휴원일 이름 (2026-09-22, v33.242~244, 원장 승인 S3·S1·S2)
- **v33.242 S3**: 월간 화면 아래의 옛 «월간 영역별 성장 비교» 패널에서 **중복 그래프 + 범례 제거**(눈금이 안 보이던 SVG `<text>` 방식) → 제목 «월별 수치 표 · 선생님용», 월×영역·출결·과제 표만 남김. 죽은 계산(`compGridYData`·`compSeriesData`·`compXLabelsData`·`_cX`·`_cY`)과 내보내기도 삭제. 그래프는 월간 카드(#monthcap) 한 곳뿐.
- **v33.243 S1**: `_upcomingSessions` 에 `closed`(=`_cdClosed`) — 보강 예약 «다가오는 수업» 목록에서 **휴원일을 빼지 않고** «휴원» 허니 꼬리표 + «휴원일이라 수업이 없어요 — 눌러서 보강 잡기» + 크림 줄로 표시(빼면 추석에 빠진 수업을 보강으로 옮길 수 없다). 고르면 `_mvPickFrom` 이 사유를 «휴원일 보강»으로 자동(이미 적은 사유는 유지).
- **v33.244 S2 휴원일 이름**: `closedDays[date].nm`(20자). `_holName(ds)` = 고정 공휴일(신정·삼일절·어린이날·현충일·**제헌절**·광복절·개천절·한글날·성탄절) + 2026·2027 설/추석 연휴·부처님오신날·대체공휴일·지방선거일 표 — **`_pubHolT` 의 2026·2027 날짜 전부에 이름이 있어야 한다(테스트가 검사)**. `_cdName` = 직접 적은 이름 → 공휴일 이름. 등하원 › 휴원일 칩에 이름(없으면 회색 «이름 없음»), 누르면 `prompt` 로 적기(`setClosedName`, 휴원 상태·표식 유지). 리포트 안내 = «9.24(목)은 추석 연휴로 휴원이에요» — 건너뛴 날의 이름이 모두 같거나 **절반 이상이 같은 이름**이면 그 이름, 아니면 날짜만. 조사는 받침에 따라 «로/으로»(ㄹ 받침은 «로»).
  ⚠️ **합치기 구멍(테스트가 잡음)**: 게이트 `mergeAppData` 의 `_cdN` 과 컴포넌트 `_cdNorm` 이 항목을 `{v,t}` 로 다시 만들며 `nm` 을 버렸다 → 둘 다 `nm` 보존. **정규화 함수가 항목을 새로 만드는 맵에 필드를 추가하면 정규화 쪽도 반드시 같이 고칠 것.** 알림톡 서버는 `v` 만 읽어 영향 없음.
- 검증 `rep241_e2e.py`(아래 패널 2항목 추가) · `s1_243_e2e.py` 3항목 · `s2_244_e2e.py` 7항목(진짜 클릭 + 입력창 + 양방향 합치기).

### 8-44. 리포트 3건 — 다음 수업 휴원일 · 월간 비교 그래프 · 회차별 기록 달력 띠 (2026-09-22, v33.240~241, 원장 신고 → 위젯 → «추천안 전부»)
- **v33.240 [버그] 다음 수업에 휴원일이 찍힘**: `_wkReport` 의 다음 수업 탐색이 `_classStart` 만 보고 `closedDays` 를 안 봤다(추석 9/24 가 «다음 수업»으로 학부모에게 나갈 뻔). → `_cdClosed(data, ds)` 면 건너뜀. **개인 보강(`mkup`)은 휴원일 검사보다 먼저** 봐서 휴원 기간 사이에 잡은 보강은 그대로 다음 수업. ⚠️ `_classStart` 는 휴원일을 모른다 — 날짜를 앞으로 훑어 «수업일»을 찾는 새 코드는 `_cdClosed` 를 같이 볼 것(§2 의 4단 확인과 같은 뿌리).
- **v33.241 ① 휴원 안내 한 줄**: 건너뛴 날 중 **그 반의 수업 요일인 날만** 모아 칩 아래 골드브라운 11px — 하루 «9.24(목)은 휴원이에요» / 여러 날 «9.24(목) ~ 9.28(월) 수업은 휴원이에요»(`s.wk.nNote`, `data-wk-note`). 휴원일에 이름이 없어(«추석») 날짜만 적는다. 안내가 있을 때만 RECENT 줄이 한 줄 높아짐.
- **v33.241 ② 월간 영역별 성장 비교(#monthcap)**: «그래프가 안 나온다»의 실체 = ⓐ 세로 눈금·월 이름이 SVG `<text>{{ }}` 라 안 그려짐(§8-27 과 같은 런타임 한계) ⓑ 세로축 0~100 고정이라 80~97점대 선 5개가 위에 붙어 겹침. → 눈금·월 이름·끝점 «종합 N» 을 **HTML 절대위치**로, 세로축은 **최저값−6 을 10단위로 내림(최대 70)~100** 으로 확대 + 범례 오른쪽 «세로축 60~100 확대» 표기(속이지 않기), 격자는 path 1개(`compGridPath`), 점은 시리즈당 path 1개(`dotPath` — SVG 안 중첩 sc-for/sc-if 제거), 종합은 맨 위 굵은 선 + 흰 속 점. SVG 안 글자 0개라 우측 패널 복사(§8-9f)에도 안전. ⚠️ 화면 아래의 옛 «월간 영역별 성장 비교»(루트 `compGridY`, `hasMonthlyComp`)는 손대지 않음.
- **v33.241 ③ 회차별 기록 = 달력 띠**: 한 줄(36px)×N → `repeat(auto-fill,minmax(64px,1fr))` 칸. 칸 = 날짜(9.3) + 그날 시험 종합(숫자, 시험 없으면 «–», 결석이면 «결석») + 지각·보강 꼬리표, 색 = 출결(출석 세이지 · 지각 허니 · 결석 코랄 · 보강 스카이), title = «날짜 · 회차 · 출결», 아래 한 줄 범례. 11회 기준 목록 ≈410px → 띠 57px. 회차 번호는 칸에서 빼고 title 로만.
- 검증 `next240_e2e.py` 4항목 · `rep241_e2e.py` 5항목(6~9월 기록 + 지각·결석·보강 + 휴원 3일 시드, 화면 DOM·세로 길이 실측).

### 8-43. 오답 시험지 옵션 · 월간 «지난 시험 대비» · 스캔 PDF 자동 자르기 실험(미채택) (2026-09-22, v33.238~239)
- **v33.238 오답 시험지 옵션**: 새 창 위 막대에 «학생당 전부/5/10/15/20문항» · «풀이 여백 없음/보통(22mm)/넓게(45mm)». 새 창에는 script 태그를 쓰지 않으므로 **여는 쪽(`_wrongSheetFill`)이 `document.write` 뒤에 select 의 `onchange` 를 붙인다** — 문항 숨김 + 머리말 `.cnt`(문항 수 · 맞은 수 / N) + 정답 쪽 `tr.ar` 가 함께 바뀜. 검증 `opt238_e2e.py`(⚠️ Playwright 가 새 창의 select 를 «안 보임»으로 판정해 `select_option` 이 안 먹음 — change 이벤트를 직접 보냄).
- **v33.239 월간 성장일지**: `monthAgg().mAcad` — 그 달에 그 학생이 본 **가장 최근 학원 시험**을 `_prevAcad` 와 비교(총점 + 공통 영역 칩). «성적» 줄 아래 골드 테두리 상자 «학원 시험 · 시험명 · 지난 시험(이름) 대비». 지난 시험이 없거나 그 달에 학원 시험이 없으면 줄 없음. 검증 `mon239_e2e.py`.
- **⛔ 스캔(그림) PDF 자동 자르기 — 실험 후 미채택**(동성고 2023 1-1 기말 스캔 10쪽): ① 쪽 전체 OCR(eng) = 3쪽 18초, 문항 번호 «1)·2)·3)» 중 1개만 검출 ② 단 왼쪽 5% 띠만 잘라 숫자 전용(whitelist `0123456789).` · psm 11) = 4쪽 4.6초, 검출률 약 50% + 신뢰도 0~30 의 거짓 검출 다수(학생 필기·체크 표시가 섞임). **이 정확도로 자동화하면 문항이 잘못 잘린다** → 넣지 않음. 스캔 시험지는 기존 «시험지에서 잘라 담기»(드래그). 다시 시도한다면 글자 인식이 아니라 AI 비전(서버 필요) 쪽. 실험 스크립트 `ocr_probe.py`·`ocr_probe2.py`.

### 8-42. 오답 흐름 후속 3건 — 모의고사 통지서 버튼 · 재시험 채점 · 지문 한 번만 저장 (2026-09-22, v33.235~237, 원장 «이어서» = R6·R4·R2)
- **v33.235 R6**: 모의고사·일반 시험 성적통지서(#examcap) 위에도 «틀린 N문항 · 오답노트에 담기 · 오답 시험지 인쇄 · 전원 오답 시험지 인쇄»(`data-notice-wrong`). `clipWrongsFromExam`·`printWrongSheet` 는 원래 시험 종류와 무관(`_esQs`·`_qR`)이라 렌더값·버튼만 추가. 모의고사는 정답해설 + 문항지 PDF 를 함께 올리면 문항 그림 45개(도표·[41~42] 지문 포함). 검증 `r6_235_e2e.py`.
- **v33.236 R4 재시험 채점**: 오답 시험지를 인쇄할 때마다 학생별 **인쇄 순서(id 목록)** 를 `localStorage his_clip_print_<sid>` 에 기억(`_rtSave` — 시험 화면 인쇄는 `ex_<시험>_<문항>`, 서랍 인쇄는 그 항목 id). 오답노트 서랍에 «재시험 채점 · 마지막으로 인쇄한 N문항 — 맞힌 번호를 누르세요 (k/N)» 번호 칩(`clip.rt`, 인쇄물의 새 번호와 같음 · title=원래 문항) → `clipSolve(id)`. **번호는 목록을 얼려 두었으므로 눌러도 밀리지 않는다.** 서랍에 없는 문항(안 담은 것)은 칩에서 빠짐. 기기 로컬 기억이라 인쇄한 기기에서 채점하는 흐름. 검증 `r4_236_e2e.py`.
- **v33.237 R2**: 공유 지문 그림을 문항마다 복제하지 않고 **묶음당 한 번** `map._p[묶음id]` 에 저장, 문항은 `pg`(묶음 id)만. 쓸 때: 오답노트 담기 = 필요한 문항만 `_qimgFull(QM, n)`(Image 두 장을 canvas 로 이어 붙임, 세로 3400px 상한) / 인쇄 = 지문 `<img>` + 문항 `<img>` 두 장을 그대로 세로 배치(합성 없음). 실측 유강중1 2.0 → 1.1MB, 모의고사 4.9 → 3.9MB(모의고사는 문항마다 자기 지문이 있어 덜 줆). ⚠️ `_qimg[시험id]` 의 키를 셀 때 `_p` 는 빼고 셀 것.

### 8-41. 문제지 PDF → 문항 그림 자동 자르기 → 오답노트 → 오답 시험지 인쇄 (2026-09-21, v33.232~233, 원장 지시)
**흐름**: 1 · 정답키에 PDF 를 올리면(어느 읽기 경로든, 1.5초 뒤 `_qimgAfterPdf`) 문항 그림이 잘려 «문항 그림 N개를 잘라 두었어요» → «틀린 N문항 오답노트에»가 카드 대신 **문제 그림**으로 담김 → «오답 시험지 인쇄»(학생) · «전원 오답 시험지 인쇄»(결과판) · 오답노트 서랍 «오답 시험지 인쇄».
- **`_qLayout(pages)`**(pdf.js 글자 좌표): ① 2단 판별 = 좌우 양쪽에 글자가 있는 줄의 80% 이상이 가운데(±5pt)를 비워 둠 ② 단별 줄 묶기(y±3) ③ 머리말·꼬리말 = 숫자를 뺀 글자가 **절반 이상의 쪽에서 같은 높이**로 반복(위 12% · 아래 20% 안, 문항·지문 머리 줄은 제외) → `footTop` ④ 문항 시작 = 줄 첫머리 «N.»/«N)» 가 **1부터 이어지고** x 가 그 단의 문항 시작 x **중앙값 ±30**(전체 최솟값을 쓰면 머리말 글자에 끌려가 2단째를 전부 놓침 — 실측) ⑤ 끝 자르기 = 마지막 문항 뒤 «정답/부록/확인사항/수고하셨» → `endCut` ⑥ 지문 머리 = «[41~42]»·«※(4~5)»(범위) 또는 «※ … 읽고/물음»(범위 없음 → 첫 문항 + 이어지는 «윗글·위 대화» 문항) ⑦ 정답해설 문서(문항 시작 줄의 절반 이상이 «N. ③»)는 문제지로 쓰지 않음.
- **`_cropQuestions(files)`**: 쪽을 1.7배로 한 번씩 렌더 → 문항 영역(다음 경계까지, 단·쪽을 넘어가면 조각 여러 개) + 지문 조각을 세로로 이어 붙임. 경계는 글자 높이값이 아니라 **렌더한 그림에서 앞 줄 밑~다음 문항 글자 위 사이의 마지막 빈 줄 묶음 가운데**(`splitY`) — 굵은 문항 번호의 높이값이 앞 줄과 겹쳐 지문 상자 아래가 잘리던 것 해결. 여백 자르기에서 **가는 세로줄(높이의 85% 이상 이어지고 양옆 3~9px 가 빈 것 = 단 구분선)** 은 무시(상자 테두리는 가로줄과 이어져 있어 해당 안 됨). 단 끝까지 간 조각의 아래 **긴 가로 괘선(위 6줄이 빈 것)** 은 걷어냄. JPEG 0.74, 세로 3000px 상한.
- **보관**: `this._qimg[시험id]`(메모리) + 클라우드 별도 행 **`clip_Q_<시험id>`**(`__hisClip` 다리 재사용) — 본체 데이터에는 `es.qimgN`(개수)만. `_qimgGet` = 메모리 → 클라우드. 그림이 없으면(빠른 입력만 한 시험·로그인 전·옛 시험) 카드로 담고 «문제지 PDF 를 다시 올리면 그림으로 바뀌어요» 안내, 나중에 PDF 를 올리고 다시 담으면 **카드가 그림으로 교체**(«재도전 성공» 표시한 것은 그대로). 항목에 `qimg:1`·`key`·`mine`·`area` 저장.
- **`_wrongSheetDoc(groups, title)`**(새 창, script 태그 없음 · 닫는 태그는 `<${'/'}`): A4 · 2단 · 문항마다 새 번호 + 작은 «원래 시험 N번» + 그림(쪽 나눔 방지), 학생별 한 묶음(이름·반·맞은 수 칸), 마지막 쪽 «정답 (선생님용)»(새 번호 → 원래 문항 → 정답 → 처음 고른 답), 위 막대 «정답 쪽 빼고 인쇄»(체크박스 + CSS) · «인쇄 · PDF 로 저장». 창은 클릭 순간 먼저 열고(팝업 차단 회피) 그림을 불러온 뒤 채움. 서랍 인쇄는 **미해결 + (문제 그림 또는 손으로 담은 사진)** 만 — 자동 카드는 정답이 적혀 있어 제외하고 몇 장 뺐는지 알림. 확정 안 된 문항(두 칸 마킹·서술형 미입력)은 시험지에서도 제외.
- **실측**(`crop_probe.py --tag=… pdf`, 그림을 폴더에 떨궈 눈으로 확인): 항도중2 30문항 1.1초·1.4MB / 유강중1 25문항 1.8초·2.0MB / 26년 9월 고1 모의고사 45문항(도표·[41~42]·[43~45] 지문 포함) 5초·4.9MB. 검증 `q232_e2e.py` 4항목 · `print233_e2e.py` 8항목(진짜 클릭 → 새 창 → 그림 로드 → PDF 2·4쪽 생성).
⚠️ 모의고사처럼 긴 지문이 여러 문항에 붙으면 그림 행이 5MB 가까이 된다(지문을 문항마다 복제). 더 줄이려면 지문을 따로 저장하고 쓸 때 합치는 구조로. 스캔(그림) PDF 는 글자 좌표가 없어 자동 자르기가 안 된다 → 기존 «시험지에서 잘라 담기»(드래그).
- **v33.234 전수 검수에서 재현·수리한 결함 5건**: ① 정답표만 있는 PDF 의 «채점 방법 1.~5.» 번호 목록을 문항으로 자름 → 자른 문항 수가 정답키 문항 수의 60% 미만(또는 +3 초과)이면 버림 ② **[예전부터 있던 구멍]** PDF 를 올리고 읽는 1~2초 사이에 다른 시험을 열면 **정답키가 그 다른 시험을 덮어씀**(`_applyAcadKey`·`_importMockKey` 가 끝난 순간의 `activeExamSetId` 를 봄) → `importExamKeyPdf` 가 올린 순간의 시험 id(`es9k`)를 끝까지 넘김 ③ 문항 그림도 같은 이유로 엉뚱한 시험에 붙던 것 → 같은 id 사용, 안내 문구는 그 시험을 보고 있을 때만 ④ 정답키 «되돌리기» 뒤에도 새 PDF 의 문항 그림이 남음 → `_qimgDrop`(메모리 + 클라우드 행 비움) + 안내 ⑤ 시험을 지워도 `clip_Q_` 행이 남음 → `_examSetDrop` 에서 비움. 검증 `fix234_e2e.py`. ⚠️ **비동기로 끝나는 작업(PDF 읽기·그림 자르기·클라우드 불러오기)은 시작할 때의 대상 id 를 잡아서 끝까지 들고 갈 것 — 끝난 순간의 state 를 보면 그 사이 화면 전환에 당한다.**
⚠️ 테스트용 항도중2 PDF 는 `_tools/hisbooks-test-pdf/` 로 옮겼다(바탕화면 원본을 원장이 정리).

### 8-40. 통지서 «지난 시험 대비» · 틀린 문항 오답노트 자동 담기 (2026-09-21, v33.230~231, 원장 승인 P7·P8)
- **v33.230 P7**: 학원 시험 통지서(#schoolcap, **캡처 영역 안** = 학부모에게 감) 영역별 성취도 아래 «지난 시험 대비 · 시험명 (날짜)» + 칩(총점 · 두 시험에 **공통인 영역만**, «80점 → 96점 ▲16», 오름 세이지 · 내림 코랄 · 같음 그레이지). `_prevAcad(dd, es, sid)` = 같은 학생이 답을 낸 **학원 시험(kind academy)** 중 날짜가 더 이른(같으면 먼저 만든) 가장 최근 것. 없으면 줄 자체가 없음. 모의고사 통지서(#examcap)는 유형이 17개라 넣지 않음. 검증 `p7_230_e2e.py` 7항목.
- **v33.231 P8**: 통지서 옆 «틀린 N문항 오답노트에»(그 학생) · 결과판 «전원 틀린 문항 오답노트에 담기» → `clipWrongsFromExam(sids, tag)`. 오답노트는 사진 기반(`clip_<sid>` 별도 행, 본체 blob 에 안 들어감)이라 **문항 카드를 canvas 로 그려 JPEG**(`_wrongClipImg`: 시험명·날짜·N번·영역·내 답→정답(서술형은 채점·모범답)·해설 4줄, 약 35KB)로 넣는다. id 는 결정론 `ex_<시험id>_<문항>` → 다시 눌러도 중복 없음. 다시 채점해 맞게 된 문항의 자동 카드는 빼되 **«재도전 성공» 표시한 카드와 손으로 담은 사진은 건드리지 않음**. 두 칸 마킹·서술형 미입력처럼 **아직 확정 안 된 문항은 담지 않음**. 저장은 기존 서랍과 같은 규칙(클라우드/로컬 중 t 최신에 이어 붙임). 자동(채점 즉시) 담기는 일부러 안 함 — 정답키 확인 전 잘못된 오답이 학생 서랍에 쌓이고 학생 수만큼 클라우드 쓰기가 생기기 때문. 검증 `p8_231_e2e.py` 8항목(진짜 클릭 · 서랍 그림 3장).
⚠️ 남은 것(제안만): 문제지 PDF 에서 **문항 그림 자체를 잘라** 카드 대신 넣기 — 문항 좌표·공유 지문·시험별 그림 저장 행이 필요한 별도 작업.

### 8-39. 정답키 되돌리기 · 결과판 «아직 채점 안 된 학생» · «문항 점검» (2026-09-21, v33.227~229, 원장 승인 P3·P5·P6)
- **v33.227 P3 되돌리기**: `_keySnap(es)` 가 정답키를 덮어쓰기 **직전**(`_applyAcadKey` 시작 · `_importMockKey` 적용부)에 `es.qPrev={q,kind,acad,grade,name,t}` 로 보관. 빠른 입력 칸 옆 «↩ 되돌리기»(`data-key-undo`, qPrev 있을 때만) → `undoExamKey` 가 현재 상태를 다시 qPrev 로 두고 복원(=한 번 더 누르면 앞으로) · `_esRegrade` 로 학생 점수까지 복구. 한 단계만 보관. 검증 `p3_227_e2e.py` 7항목(다른 PDF 오업로드 → 손으로 고친 유형·배점·점수 복구, 재실행, 빠른 입력, 모의고사 덮어쓰기).
- **v33.228 P5 미응시자**: 결과판 «학생별 결과» 아래 크림 상자 «아직 채점 안 된 학생 N명»(`acadRep.b.absent`) — **채점된 학생이 한 명이라도 있는 반**의 명단에서 퇴원·접수대기·시작 전을 뺀 나머지(반별 가나다). 시험이 «반 구분 없음»이라 기준 반을 이렇게 잡았다. 전원 채점되면 상자 없음.
- **v33.229 P6 문항 점검**(`acadRep.b.qc`, 채점 4명 이상 · 객관식만): 우선순위 ① **정답 확인** = 한 오답이 정답자보다 많고 응시자의 절반 이상(정답키 오류 탐지) ② **너무 어려움** = 정답률 20% 이하 ③ **변별 거꾸로** = 6명 이상일 때 상위 1/3 정답률이 하위 1/3 보다 34%p 이상 낮음. 전원 정답 문항은 «전원 정답 N문항: …번» 한 줄로 묶음. ⚠️ ②를 ③보다 먼저 보는 이유: 거의 다 틀린 문항에서 하위권 1명이 찍어 맞히면 ③으로 오판된다(테스트에서 실측).
- 검증 `p56_229_e2e.py` 9항목. v33.228·229 는 한 커밋.

### 8-38. 서술형 후속 3건 — 배점 따로 · 전송 전 확인 · 반 전체 입력표 (2026-09-21, v33.224~226, 원장 승인 P1·P2·P4)
- **v33.224 P1 서술형 배점**: 1 · 정답키에 서술형이 있는 학원 시험만 «서술형 N문항 배점 [ ]점씩 · 적용» 줄(`data-subj-pt`, `setSubjPt`). 서술형(복수정답 포함) 전부 v점. **객관식 배점이 하나도 없거나(=배점 표기 없는 시험지) 이전에 자동 분배한 시험(`es.acad.autoPt`)이면** 남은 점수(100 − v×N)를 0.1점 단위로 똑같이 나누고 마지막 객관식 문항으로 합계 100 을 맞춤. PDF 에서 배점을 읽은 시험은 객관식 배점을 건드리지 않음(총점은 원래대로 100 환산). v×N ≥ 100 · 숫자 아님은 거부. 적용 즉시 `_esRegrade`. △ 는 그 문항 배점의 절반. 검증 `p1_224_e2e.py` 13항목.
- **v33.225 P2 전송 전 확인**: `_sendGuard(es, sub)` — 서술형 미입력(`_subjMiss`) 또는 두 칸 마킹(`sub.amb`)이 남아 있으면 통지서 «이미지 복사·PNG 저장» 때 `confirm`(무엇이 몇 문항·지금 어떻게 계산돼 있는지 명시, 취소 권장). `#schoolcap`·`#examcap` 의 onCopy/onSave 4곳. 다 확정되면 묻지 않음. 검증 `p2_225_e2e.py`(진짜 클릭 + 대화창 취소/확인).
- **v33.226 P4 반 전체 입력표**: 2 · OMR 채점의 «서술형 한꺼번에 입력 · 빈칸 N» 버튼(빈칸 있으면 크림+골드 테두리) → 창(`data-subj-grid`): 채점된 학생(가나다) × 서술형 문항, 문항 머리에 번호·모범답(title=전체)·«빈칸 O»(`fillSubjCol` — 그 문항의 **빈칸만** O, 이미 넣은 칸은 안 건드림), 칸은 누를 때마다 **O → △ → ✕ → 빈칸**(`setExamSubj(no, r, sid)` — 셋째 인자로 학생 지정, `r=null` 은 지우기), 줄 끝에 실시간 점수, 머리에 «아직 빈칸 N개». 렌더값 `subjGrid = _subjGridVals()`(창이 닫혀 있으면 버튼 글자만 계산). 검증 `p4_226_e2e.py` 9항목(진짜 클릭 4회 순환·다른 학생 불변·성적 기록 반영·빈칸 O·완료).
⚠️ 큰 HTML 을 heredoc 으로 넘기면 bash 가 따옴표에서 깨진다 — 패치 파일은 Write 도구로 만들 것.

### 8-37. 통합 읽기 + 정답 빠른 입력 (2026-09-21, v33.222, 원장 «포맷 상관없이 스스로 읽기» → ①+② 승인)
**원칙**: 양식별 읽기를 더 늘리지 않는다. 기존 세 양식(표·쪽집게·보조 읽기)은 해설·영역 제목까지 읽으므로 그대로 먼저 시도하고, **안 맞으면 `_acadUniParse(docs)`**(v33.220 의 `_acadListParse` 를 대체·삭제).
- **통합 읽기** = «번호 + 정답» 증거 수집 후 가장 그럴듯한 묶음 선택. ⓐ 표지 모드: «정답·답안·해답·모범답·Answer» 글자가 든 줄(최대 14곳)마다 그 뒤의 표식 `N.` `N)` `N번` `N:` `[N]` `(N)` `N)④`(붙은 글자)를 읽어 1번부터 이어지는 목록을 만듦 — 앞의 동그라미 = 정답(여러 개면 복수정답), 뒤 글자 = 해설, 동그라미 없으면 서술형, 동그라미가 하나도 없고 «1) 4»꼴이 80% 이상이면 숫자를 정답으로. 같은 번호가 다시 나오면(해설 구역) 거기서 멈춤. ⓑ 짝 모드(표지 없음): 표식 바로 뒤 동그라미 · «1~5 ③②①④⑤» 범위 · «1 ③ 2 ②» 한 줄 3쌍 이상 · 번호 줄/동그라미 줄 표. 85% 이상 채워진 가장 큰 번호까지 채택. 점수 = 동그라미 정답 수(같으면 뒤쪽 표지). `cols`·`flat` 둘 다 평가.
- **스스로 점검**(확인 필요 표시): 빠진 번호 · 두 군데 정답이 다름 · 배점이 일부에만 있음 · «N문항» 표기와 다름 · 문제 번호 수와 정답 수가 다름. **정답이 없는 문제지만 있으면 못 읽음으로 끝낸다(지어내지 않음).**
- 배점·영역은 «N.» 문제 시작이 가장 길게 이어지는 문서에서(«(4점)»·«[3점]», 발문 키워드).
- **실측**: 통합 읽기 **단독**으로 실제 PDF 7종(항도중1 3종 · 유강중 2종 · 45문항 4판 · 26년 9월 고1 모의고사) 정답 전부 전용 읽기와 일치(`yg_uni.py`).
- **② 정답 빠른 입력**(`#exam-quick-key` textarea + «채우기», `fillExamKeyQuick`): 먼저 통합 읽기로(«1) ④ 2) ②» 통째 붙여넣기·서술형 글자 답), 안 되면 **한 글자 = 한 문항**(1~5·①~⑤ = 정답, «서»·S·0·-·x = 서술형, 나머지 무시, 5~45문항). `_applyAcadKey(res, keep=true)` — 이미 학원 시험이면 **배점·유형·해설은 지키고 정답만** 바꿈. 레벨테스트 세트 거부. PDF 실패 안내가 이 칸을 가리킴. ⚠️ input 은 붙여넣을 때 줄바꿈을 지우므로 반드시 textarea.
- **모의고사 우선순위**: 통합 읽기 결과(`kind:'auto'`)는 바로 쓰지 않고 `_importMockKey(files, fb)` 에 넘겨, 모의고사 읽기가 **정답을 더 적게 읽었을 때만** 채택(모의고사의 [3점]·유형 이름 보존). 덤: 히즈 레벨테스트 정답표 PDF(번호 세로 나열/동그라미 세로 나열)가 예전 0개 → 45개 정확.
- **v33.223 전수 검수에서 재현·수리한 결함 2건**(둘 다 v33.221~222 에서 내가 만든 것): A 모의고사로 채운 세트에 빠른 입력을 하면 학원 시험으로 바뀌며 유형·[3점] 배점이 지워짐 → 학원 시험이 아닌데 정답이 이미 있으면 **정답만 교체**(`_applyAcadKey` keep 분기). B 답안 합치기가 학생별 «t 큰 쪽 통째»라, 선생님이 서술형 점수를 넣은 뒤 다른 기기가 같은 학생 OMR 을 다시 스캔하면 `subj` 가 사라짐 → 이긴 쪽에 없는 `subj` 항목은 진 쪽에서 가져옴(`mergeAppData` submissions). 검증 `fix223_e2e.py`. ⚠️ 답안(sub) 안에 새 입력 칸을 만들면 합치기에서 통째 교체로 지워지지 않는지 반드시 양방향으로 볼 것.
- 검증 `uni222_e2e.py` 16항목(가상 양식 9종 + 진짜 타이핑·클릭 빠른 입력 6종).
⚠️ 그림(스캔) PDF 는 여전히 못 읽는다 → 빠른 입력이 안전망. AI 읽기(③)는 서버 기능이 필요해 미착수.

### 8-36. 정답해설 PDF 네 번째 양식 — «1) ④» 정답 목록 + 서술형·복수정답 (2026-09-21, v33.220, 원장 신고 «항도중2 모의평가도 인식 못 함»)
문제은행 편집기로 뽑은 시험지: 문제 «1. 1)다음…», **마지막 쪽 «…- 정답» 아래 «1) ④ / 5) (C)-(B)-(D)-(A) / 13) ②, ⑤ / 19) astronaut»** 목록, 배점·해설·영역 제목 없음, 문제지와 정답이 한 파일.
- `_acadListParse(docs)`: `_acadParse` 의 기존 세 양식(표·쪽집게·보조 읽기)이 **모두 안 맞을 때만** 호출(`if(!ans) return this._acadListParse(docs)`) → 기존 PDF 결과 불변(항도중1 3종 기준선 동일). «정답» 글자가 든 줄 **이후**의 «N)» 토큰만 읽음(문제 쪽의 «1. 1)»은 그 앞이라 무시), `cols` 먼저·`flat` 다음, 번호가 1부터 빠짐없이 이어질 때만 채택. 동그라미 1개 = 정답, 2개 이상 = 복수정답, 글자 = 서술형. 영역은 같은 파일의 발문으로 추정, 학년은 파일 이름 «중2/고1».
- 서술형·복수정답 문항 = `sub:1`(+`multi:1`, 모범답 `ans`). 정답키 줄에 «서술형/복수정답» 표식(`data-q-sub`, title=모범답). **그 줄에서 정답 칸을 직접 누르면** `setExamQ('key')` 가 `sub` 를 풀어 객관식으로 편입·재채점. (v33.220 은 이 문항들을 채점에서 뺐으나 배포 전 v33.221 로 대체)
- **v33.221 선생님 입력 → 총점 합산(원장 지시)**: 2 · 학생 답·채점의 서술형 줄은 ①~⑤ 대신 **O(1) · △(0.5) · ✕(0)** 칸 → `setExamSubj(no, r)` 가 `sub.subj[문항]` 에 저장(같은 칸 다시 누르면 취소)·`sub.t` 갱신·재채점·성적 기록 반영. 채점은 공용 판정 **`_qR(q, sub)`**(객관식 1/0, 서술형 = 입력 비율) 한 곳으로 — `_esGrade(es, answers, subj)`(배점 × 비율, 배점 없으면 비율 합/문항 수), 틀린 유형(△도 틀린 유형에 포함), `_acadStats`(영역 성취도에 부분 점수·문항 정답률), 통지서 답안 지도(✓/△/✕)·틀린 문항 짚어보기(«부분 점수» + 모범답).
  **미입력은 0점으로 셈 + 경고**(두 칸 마킹과 같은 원칙): `_subjMiss(es, sub)` → 학생 알약 «서술 N · 점수», 2단계 안내 줄, 통지서 위 «서술형 미입력: …번»(캡처 밖). OMR 카드를 다시 스캔해도 `subj` 는 유지. 복수정답도 같은 방식(OMR 은 한 칸만 읽으므로 선생님이 O/✕).
- 검증 `subj221_e2e.py` 15항목(실제 파일 올리기 → OMR 만 66.7 → 5번 O **진짜 클릭** 70 → △·✕ 섞어 95 → 통지서 △·✕·모범답 → 결과판 평균·7번 정답률 50% → 재스캔 유지 → 취소 → 13번 객관식 전환 91.7).
⚠️ 새 채점 코드는 `a===q.key` 를 직접 쓰지 말고 `_qR` 를 쓸 것. 부분 점수는 O/△/✕ 3단계뿐(문항별 숫자 점수 입력은 없음 — 배점 없는 시험지가 기준이라 비율로 설계).

### 8-35. ★[뿌리 확정] 되살아나는 «포항동지여고 2학년» 6명 = `seed()` 예시 명단 (2026-09-21, v33.219, 원장 내보내기 파일로 확정)
**확정 근거(실데이터 `HIS_데이터_2026.09.21.json`)**: 지운 6명 사본이 `seed()` 의 예시 명단과 글자 단위 일치 — 반 «포항동지여고 2학년», 학교 «포항동지여고», 민혜원 196·최민경 29·박지우 19·강보경 20·소승현 43·김지윤 49, **registeredAt 없음**(addRoster 는 항상 오늘 날짜를 찍으므로 명단 붙여넣기 아님), 기록 0건, `restoredStudents` 무관(1건·다른 학생). `deletedClasses` 15개·`purgedArchive` 50건 = 같은 반이 새 id 로 반복 생성·삭제된 흔적.
**경로**: 생성자 `if(!data||!data.classes) data=this.seed()` — 저장 데이터가 빈 기기(새 폰·새 브라우저·시크릿 창·카톡 인앱·저장소 비움·`?hisreset=1`)가 열릴 때마다 **새 uid 로** 예시 반 생성 → 로그인 부팅 병합(로컬+클라우드 합집합)으로 학원 데이터에 섞임 → id 가 매번 달라 삭제 표식(tombstone)이 절대 안 맞음. 8월의 «유령 반»(§8-10)·«3중 복제»(§8-21)·이번 건이 **전부 이 한 뿌리**. 그동안의 수리(_fixDupRoster·표식 강화)는 모두 증상 처리였다.
**수리**: ① `seed()` = 빈 데이터(예시 명단 삭제) ② `resetData` 빈 데이터 대응 ③ 안전망 `_stripSeed(d)`(컴포넌트: 부팅·`_absorbFresh`) + `_hisStripSeed(d)`(게이트: `mergeAppData` 끝) — 반 이름 «포항동지여고 2학년» && 학생 1~6명 전원이 (학교 «포항동지여고» · 예시 이름·마일리지 일치 · registeredAt/enrollDate/intake/fees/withdrawn/pending 없음 · records/exams/counsels 미연결)일 때만 제거. 옛 버전이 캐시된 기기가 다시 올려도 걸러진다.
**게이트 `his-ghost` ④ 5항목 추가**(빈 기기 첫 실행에 예시 반 없음 · 양방향 병합 제거 · 이름만 같은 진짜 반 보호 · 기록 연결된 예시 학생 보호) — 총 11항목. v33.218 에서 재현(빈 기기 → 6명 생성, 병합 양방향 유입) 후 수리 확인.
⚠️ **앱 코드에 실제 학생 이름이 든 예시 데이터를 두지 말 것.** ⚠️ **«지워도 되살아남»은 표식 문제이기 전에 «누가 새 id 로 다시 만드는가»부터 볼 것 — 원장 내보내기 파일의 registeredAt 유무·deletedClasses 개수가 결정적 단서였다.** 실데이터 추적 스크립트 = scratchpad `ghost_data.py`(로컬 전용, 연락처 미출력).
§8-34 의 ①③ 수리는 별개의 실제 구멍이라 유지.

### 8-34. 최민경·소승현이 계속 되살아남 — 구조 수리 (2026-09-21, v33.218, 원장 신고 3번째)
**내 실수부터**: v33.203(D5)에서 `_fixDupRoster` 를 «끝난 사고 복구»로 보고 지웠다. 그런데 §8-21 v33.118 기록대로 그 루틴은 증상을 눌러 주던 덮개였고, 뿌리는 미해결이었다. ⚠️ **«끝난 복구»를 지우기 전에 그 루틴이 막던 증상의 뿌리가 실제로 닫혔는지 먼저 확인할 것.**
**코드에서 확인된 구멍 3개**(원장 실데이터는 볼 수 없어 «무엇이 되살렸는지» 단정은 못 함 — 아래는 재현으로 확인한 것):
① `addRoster` 가드가 **재원생만** 대조(`if(s9.withdrawn) return`) → 퇴원생·보관함(`leftStudents`) 학생이 든 명단을 붙이면 새 id 사본 생성. 화면의 6명이 전부 «포항동지여고 · 마일리지 N»(명단 붙여넣기 흔적). → 퇴원·보관함 이력도 이름+`_schoolKey` 로 대조해 건너뛰고 «되살리기» 안내.
② 옛 `_fixDupRoster` 는 `intake.parentContact`/`birth` 가 있으면 «기록 있는 사본»(sc9>0)으로 보고 불간섭 → 연락처까지 붙여넣은 사본은 처음부터 정리 대상이 아니었다(삭제된 루틴이라 수리 대상 아님, 기록만).
③ 삭제(`stuDelT`)·되살리기(`stuResT`)를 **기기 시계 `Date.now()`** 로 겨룸. 시계가 앞선 기기가 되살리기 도장을 남기면 그 뒤의 삭제가 매 병합에서 짐(`dt9>rt9` 실패 → `restoredStudents` 합집합이 이김) = «지워도 계속 돌아옴». 재현: 되살리기 도장 +1시간 → 삭제 → 양방향 `mergeAppData` 모두 부활. → 삭제 도장 = `max(now, stuResT+1)`(5곳), 되살리기 도장 = `max(now, stuDelT+1)`(`_restorePut`). 나중에 한 행동이 반드시 이긴다.
**상시 게이트 `_check/his-ghost.py`**(6항목: 명단 붙여넣기 가드 3 + 도장 2 + JS오류) — 학생 삭제·복원·일괄 등록을 고치면 반드시 실행. ⚠️ LWW 도장은 기기 시계를 믿지 말 것 — 상대 도장을 알고 있으면 그보다 크게 찍는다.
남은 것: 이미 생긴 6명 사본은 자동으로 지우지 않는다(원장이 반 «삭제» → «모두 보관함으로 옮기고 반 삭제»). 실데이터 확정 진단은 원장 내보내기 파일이 있어야 가능.

### 8-33. 모의고사 정답해설 PDF 가 학원 표 양식으로 오인 (2026-09-21, v33.216, 원장 신고 «26년 9월 고1 — 41문항·정답 9개·225점»)
**원인 3겹**: ① 이 PDF 는 글자가 단어마다 떨어져 저장돼 `_pdfTwoModes` 줄이 «[ 출제의도 ]»로 나옴 → `_acadParse` 의 모의고사 판별(`'[출제의도]'` 포함 여부) 실패 ② 정답표 줄 «1 ③ 2 ② 3 ④ 4 ⑤ 5»가 표 양식 조건(첫 칸 번호 + 끝에서 둘째 동그라미 + 끝 숫자)에 맞아 1번=⑤·5점으로 읽힘 ③ 모의고사 읽기(`_parseExamKey`)도 정답표를 «정답 ~ [출제의도]» **구간 자르기**로 찾는데 제목 «고1 정답 및 해설» 직후 «4. [출제의도]»가 와 구간이 몇 글자 → 정답 0개.
**수리**: ⓐ 판별에 공백 제거 비교 추가 ⓑ 표 읽기: 가운데 칸이 전부 숫자·동그라미인 줄은 버림 ⓒ `_parseExamKey`: 구간 방식이 45개 미만이면 **줄 단위 읽기**(한 줄에 «N ③» 3쌍 이상 + 번호 연속) 결과와 비교해 더 많이 읽은 쪽 ⓓ `_importMockKey`: «2026학년도 9월 고1» → 시험명 «26년 9월 고1 전국연합학력평가»(이름 비었을 때)·학년 고1.
검증: `mock216_e2e.py`(실제 파일 입력: 45문항·정답 45·3점 10·총 100·정답표 일치·시험명·학년) · 2026년 3·6월 45/45 불변 · **2025년 9월은 예전부터 30/45 였던 것이 45/45**(pymupdf 독립 추출과 일치) · 유강중 11항목·항도중 3종 기준 동일.
**v33.217 모의고사 41~45번 유형(원장 확정 이름)**: 41 «장문(글의제목)» · 42 «장문(어휘추론)» · 43 «장문(글의순서)» · 44 «장문(지칭추론)» · 45 «장문(내용불일치)». 정답해설은 «41~42. [출제의도] A, B» · «43~45. [출제의도] A, B, C» 묶음 표기인데 예전엔 «42.»·«45.»만 보고 문자열 전체를 그 문항에 붙이고 나머지는 «장문독해»였다.
`_parseExamKey`: 묶음이면 쉼표로 나눠 문항별 배정 — 단 **조각 수가 맞고 모든 조각이 24자 이하·«하기»로 끝날 때만**(2단 PDF 는 `_pdfFullText` 가 옆 단 글자를 섞어 «글의 / 다. 기술적인 … 적절한 어휘 파악하기»처럼 나오고, 우연히 쉼표 수가 맞아 쓰레기가 채택된 적 있음). 묶음이면 단일 «N.» 읽기로 떨어지지 않음.
`_mapExamType`: 41~45 는 raw 에서 제목/지칭/순서/어휘·낱말/일치·세부·언급 중 **한 종류만** 잡히면 그것, 아니면 번호 기준(수능·전국연합 영어 고정 구성). 18~40 매핑은 그대로(24번 제목 = «주제파악» 유지). 검증 `mock217_e2e.py` 8항목.
⚠️ 채점에 쓰는 PDF 읽기를 고치면 `yg_parse.py`(앱 실제 코드로 PDF 직접 실행, `[모의고사 읽기]` 줄 포함)로 모의고사·학원 양쪽을 다 돌려 볼 것 — 한쪽 판별 실패가 다른 쪽 오인으로 이어진다.

### 8-31. 성적 성장일지 «수정»에서 시험명 고치기 (2026-09-20, v33.213, 원장 지시)
OMR 자동채점 기록은 `category = es.name`(시험 이름)·`title = ''` 이라, «수정»을 누르면 구분 Select(모의고사·학교시험·학원시험)에 맞는 값이 없어 비고 시험명 칸도 빈 채로 열려 이름을 고칠 수 없었다.
- `editExam`: category 가 3종이 아니면 **시험명 칸 ← 이름, 구분 ← `_examCat(e)` 추정**. 저장하면 기록이 `category=구분 · title=이름`(손입력 기록과 같은 꼴)이 되고 examSetId·점수는 그대로.
- 재채점 writer 2곳(`setExamAnswer`·`applyOmr`): `rec.category` 가 이미 3종(=손으로 고친 기록)이면 덮지 않음. 아직 안 고친 기록은 전처럼 시험 이름을 다시 씀(채점 창 «이름 바꾸기» 반영 경로 유지).
- 표시는 손대지 않음: 안 고친 OMR 기록은 여전히 «이름 · —», 고친 기록은 «구분 · 이름». 표시 통일(성적 탭·종합 카드·리포트 배지)은 별건(원장 판단 대기). 검증 `title213_e2e.py` 10항목.
⚠️ 시험 채점 테스트(acad_e2e·fix196_e2e)의 샘플 PDF(항도중1 쪽집게 3판 6종)는 **`C:\Users\User\Desktop\C\_tools\hisbooks-test-pdf\`**(원장 zip «26-2 항도중1 중간대비시험지.zip»에서 풀어 둠, 저장소 밖). 바탕화면 원본은 원장이 정리하고, scratchpad 는 비워질 수 있어 두 번 사라졌다 — 다시 없어지면 그 zip 에서 풀 것.

### 8-30. «종합» 보고서가 시작 전 학생에게 안 열리던 구멍 (2026-09-18, v33.212, 원장 신고)
원장 신고: «오늘 등록 확정한 임성준(내일부터 등원) — 학원 일지·선생님 일지 모두 종합이 안 열림, 반 명단엔 보임». 원인: 종합 카드 `pStu = students.find(...)` 의 `students` 는
**수업 가능 학생만**(`_teachable` = 퇴원 아님 && 시작 전 아님, v33.171 명단 정렬 때 함께 쓰이는 목록)이라 시작일이 미래인 학생·퇴원생은 profileStudentId 가 세팅돼도 카드가 안 만들어졌다.
수리: 그 목록에 없으면 반 전체 명단(`cls.students`)에서 찾음 — 일간일지 등 다른 화면은 계속 수업 가능 학생만. 신규 레벨테스트 성적통지서 위치 = **종합 → 머리 «신규 상담·진단» → #intakecap**.
검증 `profile_probe.py`(시작일 내일 학생: 종합 열림 → 신규 상담·진단 → 성적통지서 이름 확인). ⚠️ `students`(renderVals) 로 «특정 학생 한 명»을 찾는 새 코드는 시작 전·퇴원 학생도 대상인지 먼저 생각할 것.
